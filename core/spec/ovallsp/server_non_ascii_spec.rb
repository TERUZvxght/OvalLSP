...
RSpec.describe "Ovallsp::Server non-ASCII position handling (Task 008.5)" do
  ...
  it "resolves ovallsp/explainType to User (not shifted by the preceding Japanese comment/string) on the bare `user` line" do
    input =
      frame(
  ...
  it "resolves ovallsp/explainType on the line containing the emoji itself, past the emoji's 2 UTF-16 code units" do
    input =
      frame(
        jsonrpc: "2.0", method: "textDocument/didOpen",
        params: { textDocument: { uri: "file:///a.rb", text: FIXTURE_SOURCE, version: 1, languageId: "ruby" } }
  ...
  describe "ERB position correspondence with non-ASCII content" do
    it "keeps Ruby code recoverable alongside Japanese/emoji HTML content" do
      ...
      extracted = Ovallsp::Erb::RubyRegionExtractor.extract_ruby_source(source)
    ...
    # the way from didOpen through ovallsp/explainType, not just inside the
    # extractor in isolation.
    it "resolves ovallsp/explainType at the correct UTF-16 Position after several astral emoji earlier on the same .erb line" do
