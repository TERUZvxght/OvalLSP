...
RSpec.describe "Ovallsp::Server didClose index consistency (Task 008.5)" do
  ...
  it "removes a brand-new unsaved file entirely once closed" do
    Dir.mktmpdir do |root|
      uri = Ovallsp::UriUtil.from_path(File.join(root, "never_saved.rb"))
  ...
  it "no longer offers a closed-and-reverted class for definition lookup" do
    Dir.mktmpdir do |root|
      ...
      File.write(path, "class User\nend\n")
      uri = Ovallsp::UriUtil.from_path(path)
      other_uri = Ovallsp::UriUtil.from_path(File.join(root, "other.rb"))
