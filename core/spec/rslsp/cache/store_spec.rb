...
RSpec.describe Rslsp::Cache::Store do
  def summary(uri: "file:///a.rb", content_hash: "abc")
    Rslsp::Index::FileSummary.new(uri: uri, content_hash: content_hash, document_version: nil, declarations: [],
                                   diagnostics: [])
  end
  ...
  it "returns nil rather than raising when the cache directory itself is unusable" do
    store = described_class.new(cache_dir: "/nonexistent-parent-dir-#{Process.pid}/rslsp-cache")
