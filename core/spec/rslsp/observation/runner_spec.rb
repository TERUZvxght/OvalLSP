...
RSpec.describe Rslsp::Observation::Runner do
  ...
  it "observes a real, separately-spawned Ruby process running the workspace's own test command" do
    ...
    expect(signature.parameter_types).to eq([Rslsp::Types::Nominal.new(name: "Integer"), Rslsp::Types::Nominal.new(name: "Integer")])
    expect(signature.return_type).to eq(Rslsp::Types::Nominal.new(name: "Integer"))
  end
  ...
  it "never lets the spawned process' stdout/stderr reach this process' own real stdio" do
    captured = Tempfile.new("rslsp-observation-runner-stdio")
