...
RSpec.describe Rslsp::Observation::Collector do
  ...
  it "records an instance method's observed parameter and return classes" do
    ...
    expect(signature.parameter_types).to eq([Rslsp::Types::Nominal.new(name: "String"), Rslsp::Types::Nominal.new(name: "String")])
    expect(signature.return_type).to eq(Rslsp::Types::Nominal.new(name: "String"))
  ...
  it "records a call that raises without fabricating a return type, but still counts the sample" do
    ...
    expect(signature).not_to be_nil
    expect(signature.samples).to eq(1)
    expect(signature.return_type).to eq(Rslsp::Types::UNKNOWN)
  end
  ...
  it "records a singleton method call with the class itself as owner" do
    ...
    expect(signature).not_to be_nil
    expect(signature.parameter_types).to eq([Rslsp::Types::Nominal.new(name: "String")])
  end
