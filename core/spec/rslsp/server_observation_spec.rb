...
RSpec.describe "Rslsp::Server runtime observation (Task 019)" do
  ...
  def build_server(input_string, workspace_root: fixtures_root)
    Rslsp::Server.new(input: StringIO.new(input_string), output: output, logger: logger, workspace_root: workspace_root)
  end
  ...
  it "runs the configured test command and makes its evidence available via showTypeEvidence" do
    calculator_uri = Rslsp::UriUtil.from_path(File.join(fixtures_root, "lib", "calculator.rb"))
    calculator_source = File.read(File.join(fixtures_root, "lib", "calculator.rb"))

    input =
      frame(jsonrpc: "2.0", id: 1, method: "initialize", params: {}) +
      did_open(calculator_uri, calculator_source) +
      frame(jsonrpc: "2.0", id: 2, method: "rslsp/runObservedTests",
            params: { testCommand: ["ruby", "run_tests.rb"] }) +
      frame(jsonrpc: "2.0", id: 3, method: "rslsp/showTypeEvidence",
  ...
  it "returns nil from showTypeEvidence when nothing has been observed" do
    ...
    input =
      frame(jsonrpc: "2.0", id: 1, method: "initialize", params: {}) +
      did_open(calculator_uri, calculator_source) +
      frame(jsonrpc: "2.0", id: 2, method: "rslsp/showTypeEvidence",
            params: { textDocument: { uri: calculator_uri }, position: { line: 3, character: 6 } }) +
      frame(jsonrpc: "2.0", method: "exit", params: nil)
  ...
  it "clears observed evidence via clearObservedTypes" do
    ...
    input =
      ...
      frame(jsonrpc: "2.0", id: 2, method: "rslsp/runObservedTests",
            params: { testCommand: ["ruby", "run_tests.rb"] }) +
      frame(jsonrpc: "2.0", id: 3, method: "rslsp/clearObservedTypes", params: nil) +
      frame(jsonrpc: "2.0", id: 4, method: "rslsp/showTypeEvidence",
            params: { textDocument: { uri: calculator_uri }, position: { line: 3, character: 6 } }) +
      frame(jsonrpc: "2.0", method: "exit", params: nil)
