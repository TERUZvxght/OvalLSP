...
RSpec.describe "Rslsp::Server textDocument/prepareRename and textDocument/rename (Task 016)" do
  ...
  it "refuses (null result) prepareRename on a generated route helper call" do
    route_registry = Rslsp::Routes::RouteRegistry.from_route_facts([
    ...
    server = Rslsp::Server.new(input: StringIO.new(input), output: output, logger: logger, route_registry: route_registry)
    server.run
