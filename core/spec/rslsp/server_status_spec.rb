...
RSpec.describe "Rslsp::Server environment status (Task 020)" do
  ...
  it "reports ready-rails when the Runtime Agent is live and responding" do
    ...
    agent_manager = instance_double(Rslsp::AgentProcessManager, ready?: true)
  ...
  it "reports agent-unavailable when a Runtime Agent bootstrap was attempted but isn't responding" do
    server = build_server(frame(jsonrpc: "2.0", method: "exit", params: nil))
    agent_manager = instance_double(Rslsp::AgentProcessManager, ready?: false)
    server.instance_variable_set(:@agent_manager, agent_manager)
  ...
  it "acknowledges rslsp/restartAgent without raising, even with no Agent bootstrap configured" do
    input =
      frame(jsonrpc: "2.0", id: 1, method: "initialize", params: {}) +
      frame(jsonrpc: "2.0", id: 2, method: "rslsp/restartAgent", params: nil) +
  ...
  it "acknowledges rslsp/reindexWorkspace" do
    input =
      frame(jsonrpc: "2.0", id: 1, method: "initialize", params: {}) +
      frame(jsonrpc: "2.0", id: 2, method: "rslsp/reindexWorkspace", params: nil) +
      frame(jsonrpc: "2.0", method: "exit", params: nil)
