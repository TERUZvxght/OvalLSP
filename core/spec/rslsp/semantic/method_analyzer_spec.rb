...
RSpec.describe Rslsp::Semantic::MethodAnalyzer do
  ...
  let(:summary_store) { Rslsp::Semantic::MethodSummaryStore.new }
  subject(:analyzer) do
  ...
  def method_symbol(owner, name, singleton: false)
    Rslsp::Index::SymbolId.new(kind: singleton ? :singleton_method : :instance_method, owner: owner, name: name,
                                discriminator: nil)
  end
  ...
  it "unions multiple return paths (implicit fall-through plus an early explicit return)" do
    ...
    expect(summary.return_type).to eq(Rslsp::Types.normalize_union([Rslsp::Types::Nominal.new(name: "User"), Rslsp::Types::NIL]))
  end
  ...
  it "resolves self to the owner's Nominal type in an instance method" do
    ...
    expect(summary.return_type).to eq(Rslsp::Types::Nominal.new(name: "::Widget"))
  end
  ...
  it "resolves self to ClassOf[Owner] in a singleton method" do
    ...
    expect(summary.return_type).to eq(
      Rslsp::Types::Generic.new(name: "ClassOf", type_arg: Rslsp::Types::Nominal.new(name: "::Widget"))
    )
  end
