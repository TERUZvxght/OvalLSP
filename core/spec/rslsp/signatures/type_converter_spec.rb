...
RSpec.describe Rslsp::Signatures::TypeConverter do
  def rbs_type(source)
  ...
  describe ".convert" do
    ...
    it "converts an optional type to a Union with nil" do
      ...
      expect(result).to eq(Rslsp::Types.normalize_union([Rslsp::Types::Nominal.new(name: "String"), Rslsp::Types::NIL]))
    end

    it "converts a union type to a Union" do
      result = described_class.convert(rbs_type("::Integer | ::String"))

      expect(result).to eq(Rslsp::Types.normalize_union(
                              [Rslsp::Types::Nominal.new(name: "Integer"), Rslsp::Types::Nominal.new(name: "String")]
                            ))
    end
    ...
    it "converts a type variable to a TypeParameter" do
      ...
      expect(described_class.convert(variable_type)).to eq(Rslsp::Types::TypeParameter.new(name: "Elem"))
    end

    it "converts untyped to Unknown" do
      expect(described_class.convert(rbs_type("untyped"))).to eq(Rslsp::Types::UNKNOWN)
    end

    it "converts nil to the NIL type" do
      expect(described_class.convert(rbs_type("nil"))).to eq(Rslsp::Types::NIL)
    end

    it "converts a proc type to a ProcType" do
      result = described_class.convert(rbs_type("^(::Integer) -> ::String"))

      expect(result).to eq(Rslsp::Types::ProcType.new(
                              parameters: [Rslsp::Types::Nominal.new(name: "Integer")],
                              return_type: Rslsp::Types::Nominal.new(name: "String")
                            ))
    end
    ...
    it "never raises for a type it doesn't recognize, widening to Unknown instead" do
      ...
      expect(described_class.convert(nil)).to eq(Rslsp::Types::UNKNOWN)
    end
  end
end
