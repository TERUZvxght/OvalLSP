...
# anything from the `rslsp` gem by name, only by absolute path relative
# to itself, so it works whether or not the target project's own
# Gemfile happens to declare `rslsp` as a dependency at all. Everything
...
module Rslsp
  module Observation
    ...
    module Harness
      ...
      def install
        workspace_root = ENV.fetch("RSLSP_OBSERVATION_WORKSPACE_ROOT", nil)
        output_path = ENV.fetch("RSLSP_OBSERVATION_OUTPUT_PATH", nil)
        run_id = ENV.fetch("RSLSP_OBSERVATION_RUN_ID", nil)
...
Rslsp::Observation::Harness.install
