...
索引化した時点からメソッドが変更されたかどうかを判定するために必要だから
です。各ファイルの絶対パスも保存します。いずれもどこにも送信されず、
...
- 何回の呼び出しが寄与したか(例外を送出した呼び出しは何も寄与せず、
  「例外を送出した」という記録も残りません);
...
- ログファイル。ここには**あなたのテストコマンド自身の標準出力と標準
  エラーがリダイレクトされます**。OvalLSPはこのログを読んだり索引化したり
  しません——実行が失敗した際に理由を報告するために存在します——が、
