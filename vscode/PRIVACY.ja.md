...
無く、拡張機能を入れている限り溜まり続けていました。直近8つを残します。
ディスク上から*消えた*ワークスペースだけは例外で、0.2.2 からは、そのワークス
ペースを最後に開いた時点から30日間保持し、その後の最初の起動で削除します。
マウントされていないボリュームと削除されたプロジェクトは、ここからは見分けが
つきません。外付けドライブ上のプロジェクトがキャッシュを保てるのはこの30日の
おかげであり、その代償として、削除したプロジェクトのキャッシュが最大1か月
残ります。気になる場合はご自身でディレクトリを削除してください。**このキャッシュには
