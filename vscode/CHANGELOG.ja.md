...
- 修正: コンテナの値(`Hash[Unknown]`、`Array[String]` など)に対して、
  定義へ移動と補完が機能するようになりました。従来はそこでは何も返さず、
  同じクラスに別経路で到達した場合には機能していました。
...
レシーバしか理解しないため、コンテナの値に対しては定義へ移動も補完も何も
返していませんでした。これは以前からそうでしたが、今回の変更でその状態に
到達する頻度が上がるところでした。リゾルバは、ジェネリックなレシーバを
