...
| Ruby 4.0.x | best effort(gateしないCIジョブが毎pushでCoreスイートを4.0で実行し、実行できたexample数を報告する。4.0固有の失敗は記録のみで修正しない) |
...
します。0.2.0で追加したセマンティックハイライトだけは例外です。ドキュメントの
セマンティックトークンについて、VS Codeは統合ではなく単一のproviderに解決する
ため、両方を有効にしている場合の配色は片方のものになり、両方が混ざることは
ありません。どちらになるかは、どちらの拡張機能も決められません。具体的にこのテストでは: completionとgo-to-definitionのいずれも、
...
6. 永続キャッシュは `$XDG_CACHE_HOME/ovallsp/`(未設定または空なら
   ...
   **OvalLSPのバージョン**の組合せごとに分離されている。該当ディレクトリを
   ...
   組合せのディレクトリは起動時に掃除され、直近8つを残す。0.2.3 からは
   背後で掃除するので、アップグレード後の初回起動がそれを待つことはない。
   ディスクから消えたワークスペースは、最後に開いた時点から30日間残す —
   マウントされていないボリュームはここからは同じに見えるため。詳細は
