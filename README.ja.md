...
によるもので、0.2.0でその3つとも出ています。プロジェクト全体の診断(以前は開いて
いないファイルの誤りが見えませんでした)、hoverでのドキュメント表示(以前のhoverは
「それが何か」しか言わず「何のためか」を言いませんでした)、セマンティック
ハイライト(Rubyの`foo`はローカル変数かメソッド呼び出しか曖昧で、エンジンは判別
できているのにエディタが知りませんでした)。まだバージョンが入っている行も同じ
