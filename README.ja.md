...
| 識別子を打ち始めた時点での補完 — 宣言済みクラス、スコープ内のローカル変数、その位置で呼べるメソッド | 0.2.0 | ⚠️ 0.2.0 | ⚠️ 1.0.0 |
...
| 診断: 一度も代入されない `@ivar` の読み取り | 0.2.0 | ⚠️ 0.2.0 | ⚠️ 1.0.0 |
| シグネチャヘルプ: ワークスペース・標準ライブラリ・ルートヘルパー | ✅ | ⚠️(ルートヘルパーは―) | ⚠️ 1.0.0 |
| 参照検索・リネーム・シンボル検索 | ✅ | ⚠️ | ⚠️ 1.0.0 |
| 診断: 引数の**型**違い | 0.2.0 | ⚠️ 0.2.0 | ⚠️ 1.0.0 |
| 診断: 開いているファイルだけでなくプロジェクト全体 | 0.2.0 | ⚠️ 0.2.0 | ⚠️ 1.0.0 |
| hover・補完でのドキュメント(RDoc/YARD)表示 | 0.2.0 | ⚠️ 0.2.0 | ⚠️ 1.0.0 |
| セマンティックハイライト(ローカル変数とメソッド呼び出しの区別)。`.rb` とERBテンプレートのRuby領域の両方 | 0.2.0 | ⚠️ 0.2.0 | ⚠️ 1.0.0 |
...
リリースでまとめて出ます。上位3つはPylance(動的型付け
