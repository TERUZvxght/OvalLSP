...
| 診断: gemを継承したクラスでの未定義メソッド・変数 | ✅ | ⚠️ | ⚠️ 1.0.0 |
...
| 診断: 開いているファイルだけでなくプロジェクト全体 | ✅ | ⚠️ | ⚠️ 1.0.0 |
| hover・補完でのドキュメント(RDoc/YARD)表示 [^doc] | ✅ | ⚠️ | ⚠️ 1.0.0 |
| セマンティックハイライト(ローカル変数とメソッド呼び出しの区別)。`.rb` とERBテンプレートのRuby領域の両方 | ✅ | ⚠️ | ⚠️ 1.0.0 |
| 補完: Active Record の `Relation` API(`where`・`order`・`limit`) | ✅ | ―(Runtime Agentなし) | ― |
| インレイヒント(推論した型・引数名) | ✅ | ⚠️ | ⚠️ 1.0.0 |
| コードアクション/クイックフィックス(3種) | ✅ | ⚠️ | ⚠️ 1.0.0 |
| 型定義へ移動 | ✅ | ⚠️ | ⚠️ 1.0.0 |
| コールヒエラルキー(呼び出し元・呼び出し先) | ✅ | ⚠️ | ⚠️ 1.0.0 |
| ドキュメント内ハイライト(ファイル内の出現箇所) | ✅ | ⚠️ | ⚠️ 1.0.0 |
| `@` を打った時点でのインスタンス変数名の補完 | ✅ | ⚠️ | ⚠️ 1.0.0 |
| 検査ごとの診断severity設定 [^severity] | 0.4.0 | ⚠️ 0.4.0 | ⚠️ 1.0.0 |
| `require` の自動挿入 [^autorequire] | — | 0.4.0 | ⚠️ 1.0.0 |
| シグネチャヘルプ: 活性引数のハイライト [^activeparam] | 0.4.0 | ⚠️ 0.4.0 | ⚠️ 1.0.0 |

[^severity]: safe モードで有効な検査を格下げ・抑制する設定です。unresolved-constant の有効化や重大度の引き上げはできません。設定変更は open / closed の両方へ反映し、解除で既定値へ戻します。

[^autorequire]: plain Ruby のトップレベルまたは明示的なルート参照で、workspace の同名宣言と衝突しない JSON・URI・Pathname に限定します。診断は不要です。Rails、bundle / Ruby 選択環境、構文や挿入位置が不明な場合は辞退します。現在の client は古い CodeAction の適用を防がないため、編集後は改めて提案を取得してください。[能力表](docs/EXTENSION_CAPABILITIES.ja.md)も参照してください。

[^activeparam]: キーワード引数は順序によらず名前で対応させます。過剰・未知・曖昧な引数に強調範囲を返さず、署名の文字列は表示します。

[^argcount]: 壊れたら落ちるテストで検証しており、実際に発火もします。ただし
    Rubyの標準ライブラリ・Railsの5つのgem・minitest に対して出す14件は、
    すべて誤りです。原因はいずれも記録済みで、`def Const.method` が
    インスタンスメソッドとして記録される件(14件中10件)、ブロックの `self` が
...
[^rename]: マクロが宣言したメソッド(`attr_accessor :name`、
    `delegate :title, to: :author` など)はリネームせず拒否します。マクロの
    引数はマクロが読み取るソースであってメソッド名ではなく、書き換えると
    `attr_*` が返す ivar、`attr_accessor` が同時に宣言する2つ目のメソッド、
    `enum` がスコープ名と保存値の対応付けに使うラベル、`delegate` が委譲先で
    呼ぶメソッドまで変わってしまうためです(024.28)。

[^doc]: hoverでは、ポップアップがメソッドを名指す場所すべてです。
    `widget.charge` も、レシーバなしの呼び出しも、`def` 自体も対象です。
    ERBテンプレートの中だけは型のみでドキュメントは出ません。補完では `.` が出した
...
正確に名指ししており、`0.2.x` のような範囲表記は使いません。minorとは「新しく
告知するもの」が届くリリースだからです。ただしpatchでも行が ✅ に変わることは
あります——前のリリースが当然やっていると理解されていて実際にはやっていなかった
ことを指す行であれば。0.2.1 は4回それをしました。同じバージョンを共有する行は、
0.1.6が5つ同時に出したように、そのリリースでまとめて出ます。
...
未定義メソッドの行は、祖先がすべて既知のレシーバでのみ発火します。ワークスペースの
クラス、Active Recordモデル、そして0.3.0以降はgemを継承したクラスです — 動いている
アプリケーション自身のクラスが何を定義しているかをRuntime Agentが報告するからです。
この3つめ(`ApplicationController`、したがって大半のコントローラやジョブ)は、0.2.18まで
意図的に黙っていました。そこで報告することは推測を意味したためです。gemが実際に定義
しているものをインデックスすることがこの制約を解くもので、それは0.3.0で実装されました。
動いているアプリケーション自身のクラス一覧があるからこそ、そうしたレシーバも「分かる」
ものになります。したがって現在は**報告され**、上の行に0.3.0はもう入っていません。
