...
| 診断: 引数の個数違い | ✅ | ⚠️ | ⚠️ 1.0.0 |
| 診断: gemを継承したクラスでの未定義メソッド・変数 | 0.3.x | ⚠️ 0.3.x | ⚠️ 1.0.0 |
| 診断: 一度も代入されない `@ivar` の読み取り | 0.2.x | ⚠️ 0.2.x | ⚠️ 1.0.0 |
| シグネチャヘルプ: ワークスペース・標準ライブラリ・ルートヘルパー | ✅ | ⚠️(ルートヘルパーは―) | ⚠️ 1.0.0 |
| 参照検索・リネーム・シンボル検索 | ✅ | ⚠️ | ⚠️ 1.0.0 |
| 診断: 引数の**型**違い | 0.2.x | ⚠️ 0.2.x | ⚠️ 1.0.0 |
| 診断: 開いているファイルだけでなくプロジェクト全体 | 0.2.x | ⚠️ 0.2.x | ⚠️ 1.0.0 |
| hover・補完でのドキュメント(RDoc/YARD)表示 | 0.2.x | ⚠️ 0.2.x | ⚠️ 1.0.0 |
| セマンティックハイライト(ローカル変数とメソッド呼び出しの区別)。`.rb` とERBテンプレートのRuby領域の両方 | 0.2.x | ⚠️ 0.2.x | ⚠️ 1.0.0 |
| インレイヒント(推論した型・引数名) | 0.3.x | ⚠️ 0.3.x | ⚠️ 1.0.0 |
| 各診断に対応するコードアクション/クイックフィックス | 0.3.x | ⚠️ 0.3.x | ⚠️ 1.0.0 |
| 型定義へ移動 | 0.3.x | ⚠️ 0.3.x | ⚠️ 1.0.0 |
| ドキュメント内ハイライト(ファイル内の出現箇所) | 0.3.x | ⚠️ 0.3.x | ⚠️ 1.0.0 |
| コールヒエラルキー(呼び出し元・呼び出し先) | 0.3.x | ⚠️ 0.3.x | ⚠️ 1.0.0 |
| 検査ごとの診断severity設定 | 0.4.x | ⚠️ 0.4.x | ⚠️ 1.0.0 |
| `require` の自動挿入 | 0.4.x | ⚠️ 0.4.x | ⚠️ 1.0.0 |
| シグネチャヘルプ: 活性引数のハイライト | 0.4.x | ⚠️ 0.4.x | ⚠️ 1.0.0 |

バージョンが入っている行は、どの環境でもまだ実装していません。数値は実装予定の
リリースで、利用者が早く気づくものから並べています。上位3つはPylance(動的型付け
言語の言語サーバとして最も知られた基準)との比較によるものです。プロジェクト全体の
診断(現状、開いていないファイルの誤りは見えません)、hoverでのドキュメント表示
(現状のhoverは「それが何か」しか言わず「何のためか」を言いません)、
...
インデックスすることがこの制約を解くもので、上の行が0.3.xである理由です。

逆の事象が、本リリースの既知の誤検知です。gemのクラスを**再オープン**している
ワークスペースのファイルは、そのクラスを定義しているファイルと見分けがつきません。
すべてのRailsアプリケーションが持つ `test/test_helper.rb` の
`class ActiveSupport::TestCase` がこれにあたり、祖先が完全に既知だと読めてしまうため、
gem自身のAPI(`parallelize`、`fixtures`)の呼び出しが未定義として報告されます。
1プロジェクトあたり1ファイル・2件です。0.1.7で修正します
...
「―」は、Runtime Agentしか供給できないRails由来のデータで成り立つ機能です。
未信頼のワークスペースではAgentを意図的に起動せず、Railsアプリでなければ
報告する対象がありません。壊れているのではなく、設計上存在しません。
...
を記述し、`core/spec/e2e/capabilities_spec.rb` と
`vscode/scripts/verify-installed-extension.sh` が検証します。
