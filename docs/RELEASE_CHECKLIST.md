...
| 2 | Core full RSpec failure/pending 0 | ✅ `make-final-review-bundle.sh`の"Core full RSpec"ステップ。895 examples / 0 failures(Task 023.2のBuildManifest/ProtocolVersion specを含む) |
| 3 | Real Rails integration failure/pending 0 | ✅ 同スクリプトの"Mandatory real Rails integration"ステップ |
| 4 | VS Code unit/integration failure 0 | ✅ `test:unit`(65 examples、Task 023.2-023.5のversionInfo/clientLifecycle/platformCompatibility分含む)・`test:integration` |
...
| 15 | `vsce ls --tree`内容検査PASS | ⚠️ `make-final-review-bundle.sh`の"Package contents inspection"ステップでmkmf.log等のテキストベースのリーク(Task 023.6で修正済み)はhard failで検出する。**ただし**、`prism.bundle`/`rbs_extension.bundle`自体が、このビルドマシンのrbenv libruby絶対パスを`LC_LOAD_DYLIB`として埋め込んでいることを`otool -L`で確認済み(Task 023.8) — これはビルドログ副産物ではなくコンパイル済みバイナリ自身の動的リンク依存であり、安全な書き換え方法を本セッション内で検証できていない。現在は非致命的warningとして報告するのみ。この埋め込みパスが、エンジン/バージョン/プラットフォームが一致する別マシン上での動的リンク解決に影響するかは**未検証**(下記KNOWN_LIMITATIONS.mdおよびTask 023.8参照) |
