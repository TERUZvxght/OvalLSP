...
| # | 項目 | 状態 | 根拠/備考 |
|---|---|---|---|
| 1 | all unit/component/integration tests green | ✅ 判定可能・green | `core/`: 3,234 examples, 0 failures(`bundle exec rspec --order random`)。`vscode/`: `test:unit` / `test:integration`(source Core)/ `test:integration:packaged`(packaged Core)、いずれも0 failures — 後ろ2つは ci.yml の `vscode-integration` ジョブが両方実行し、どちらにも「examples が0件なら落とす」ガードが付いています。**core側の数は `core/spec/meta/documented_counts_spec.rb` が実行中のスイートと突き合わせます** — 890/895/1,776/1,833 と三度陳腐化し、「毎回測り直すこと」と書いた行自体がまた陳腐化したため、覚えておくのをやめて検査させることにした。vscode側の数はここから消した。増える数字を2箇所に書く理由がない |
...
2. 両言語の changelog → `scripts/check_changelog.rb`(preflight。バンプ前の
   バージョンについて問うのは `bump` で、`--version` を付けて走らせる)。
...
**commit は順序の一部**であって前後の片付けではない。`open` の後に
両言語の changelog と両ロードマップの節を書いて commit し、`bump` の
後にバージョン更新を commit してから `gate` に入る — `gate` は dirty
なツリーを拒否する。gate したものと publish するものを同じにするため
で、uncommitted のまま gate すると `release.sh` が publish 時に dirty
tree を拒否し、そこで commit すると index が動いて gate が無効になる。
...
| 5 | packaged integration failure 0 | ✅ `test:integration:packaged` — **0.2.17 から ci.yml の `vscode-integration` ジョブが実行します**。それまでは6文書がこの名前を挙げ、実行するものが無い状態で本行が ✅ でした(`024.125`)。実行するのは `ubuntu-latest` なので、検証されるのは**Core を拡張内に配置した読み込み経路**であって darwin-arm64 のパッケージングではありません — そちらは 0.3.2 から ci.yml の `Packaged Core (darwin-arm64)` ジョブが push ごとに vendoring と `vsix_semantic_smoke.rb` を行い、publish 時にも同じ smoke が走ります。パッケージ済み Core に対して capability suite(1 層目)を回すことは未着手で、`docs/EXTENSION_CAPABILITIES.md` が両言語でそう開示しています |
