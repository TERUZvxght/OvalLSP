...
| 8 | licenses and third-party notices | ✅ 判定可能・green | `scripts/generate_sbom.rb`で`docs/SBOM.md`を生成(RubyGems: prism/rbs、npm: vscode-languageclientのバージョン+ライセンス)。VSIXが実際に同梱するruntime依存のみが対象で、devDependenciesは対象外 |
