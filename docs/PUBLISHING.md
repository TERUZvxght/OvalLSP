...
|---|---|---|
| patch (`0.1.**7**`) | Nothing new is announced — the release makes the extension do what the previous one already claimed | Bug fixes, performance, refactoring, documentation. **A capability row may be added or turned ✅** when it names something the previous release was already understood to do and did not: 0.2.1 added `S4`, `G17`, `C14` and `W5` this way. What it must not do is announce a capability nobody was promised |
