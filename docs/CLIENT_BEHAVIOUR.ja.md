...
| **`textDocument/prepareRename` への `null` は、その位置での rename が無効だという意味である。** prepare は `textDocument/rename` の手前の門であり、`prepareSupport` を宣言するクライアントは先に これを尋ねる。仕様が null に与えている意味そのものが「そこでの rename は無効」である。`Server#prepare_rename_result` が `nil` を返すのはこのためであり、意図的に索引を温めずに拒否する判断もここに乗っている。**checked にはしていない**: この行を書いた時点で `vscode/node_modules` が入っておらず、VS Code の rename 入力欄が実際に開かないかどうかは誰も読んでいない。エンジンが依拠しているのはプロトコルの意味であって特定エディタの UI ではなく、確認済みであるかのように書かないためにそう明記する。 | LSP 仕様の `textDocument/prepareRename` (`Range \| { range, placeholder } \| { defaultBehavior } \| null`)、およびクライアント能力 `rename.prepareSupport` | |
