...
| バージョン | 区分 | 根拠 |
|---|---|---|
| 3.4(3.4.5、3.4.7で実行確認) | supported | `core/`のテストスイート(1,943 examples。`core/spec/meta/documented_counts_spec.rb` がこの数をその時々の実行と突き合わせます。895 が六リリース、次に枝の途中で採った 1,776、次に2コミットを残して採った 1,833、と三度陳腐化したためです)を3.4.7(主要)および3.4.5(rbenv経由)で実行しgreenを確認。これはリリースごとに最終ゲート(`RELEASE_CHECKLIST` #1 と CI の 3.4 ジョブ)で取り直す確認であり、この行が単独で保ち続けられる事実ではありません — 0.2.3 の準備は 3.3.6 で行われ、3.4.x での実行はリリース PR の CI とメンテナの公開前ゲートが担います |
...
| 4.0(最後に4.0.6で実行、0.2.1時点) | best effort | **この行の数値は履歴です。** 0.2.3を準備したマシンには`ruby@4`が入っておらず、0.2.1以降スイートを4.0で再実行していません(その間にexampleは百件以上増えています)。現時点の事実は、gateしないCIジョブが毎pushで実行し実行できたexample数を報告するため、「何も走らなかった」が合格として読めなくなったことです。最後に測った時点では`core/`のスイートは4.0.6でgreenでした。ただし実Railsの統合例は、このマシンの4.0.6にRailsとsqlite3が入っていないためpendingになります。**VSIXがこの上で何をするか**: 同梱のnative extensionは単一Rubyバージョン向けなのでここでは使われません。拡張機能は*利用者自身の*Rubyが `prism` と `rbs` を持っているかを確認し、持っていればそちらで動きます。持っていればOutputチャンネルへの1行、持っていなければ `gem install prism rbs` を示すエラーです。0.2.1 まではどちらでもエラーで、それは同梱物の事情を利用者の状況として伝えていました。`.github/workflows/ci.yml` は push と pull request のたびに 4.0 でも回しますが、**報告するだけでゲートしない**ジョブです — 0.2.x 系では 4.0 固有の失敗は記録するにとどめ修正しない方針で、必須ジョブにするとその方針が守れなくなるためです。実Rails を要する例は、ここと同じ理由でそちらでも skip されます |
