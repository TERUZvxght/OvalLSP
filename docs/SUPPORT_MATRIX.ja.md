...
| 4.0(最後に4.0.6で実行、0.2.1時点) | best effort | **この行の数値は履歴です。** 0.2.3を準備したマシンには`ruby@4`が入っておらず、0.2.1以降スイートを4.0で再実行していません(その間にexampleは百件以上増えています)。現時点の事実は、gateしないCIジョブがすべてのpull requestと`main`へのpushで実行し、実行できたexample数を報告するため、「何も走らなかった」が合格として読めなくなったことです。最後に測った時点では`core/`のスイートは4.0.6でgreenでした。ただし実Railsの統合例は、このマシンの4.0.6にRailsとsqlite3が入っていないためpendingになります。**VSIXがこの上で何をするか**: 同梱のnative extensionは単一Rubyバージョン向けなのでここでは使われません。拡張機能は*利用者自身の*Rubyが `prism` と `rbs` を持っているかを確認し、持っていればそちらで動きます。持っていればOutputチャンネルへの1行。持っていなければ、0.2.10 以降は**Core Serverを起動しません** — その旨のエラー通知と、Outputチャンネルに理由が出ます。どのみち `require` で失敗するため、「信頼するなと言われる応答」を出すより何も出さない方が良い、という判断です(`024.55`)。0.2.10 までは起動してしまい、通知は `gem install prism rbs` を示していました。0.2.1 まではどちらでもエラーで、それは同梱物の事情を利用者の状況として伝えていました。`.github/workflows/ci.yml` はすべての pull request と `main` への push のたびに 4.0 でも回しますが、**報告するだけでゲートしない**ジョブです — 4.0 固有の失敗は、開いたまま残さずリリースを付けて記録します — その唯一の件だった `024.288` は 0.3.2 で修正済みです。ジョブ自体はゲートしないままです: リリースを定めることは1件の報告を直す約束であって、4.0 を必須にする約束ではなく、必須ジョブにすれば最初の失敗が「誰も合意していない作業」になる点は変わりません。実Rails を要する例は、ここと同じ理由でそちらでも skip されます |
