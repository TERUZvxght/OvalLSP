...
ここへ戻ってください。** 0.1 と 0.2 はメンテナの言葉をほぼそのまま写した
もので、この文書で最も動かしてはいけない部分です。

（「以下のすべてがこの節に資する限りで価値を持つ」という強い読み方も
できますが、それは本プロジェクトの推論です。エージェントが何を省くかを
決めるときに都合よく働く種類の推論なので、指針として扱い、免罪符として
は扱わないこと。）
...
したがって優先順位は次のようになります。**メンテナが名指ししたのは型
チェックと未定義メソッド呼び出しの 2 つ、および「Pylance を参考に基礎を
盤石に」という方針までです。**表の 2 行目（それらが依拠する下位機構）は
そこからの推論であり、「中核（推論）」と記しています。

| | 内容 | 版 |
|---|---|---|
| 中核 | 型チェック、未定義メソッド呼び出しの検出、その正確さ（誤検出と見逃しの両方） | 1.0.0 |
| 中核（推論） | 上記が依拠する receiver 型追跡・メソッド解決・Rails DSL 由来の宣言 | 1.0.0 |
...
**ここから下の表は本プロジェクトの運用規則であって、メンテナの裁定では
ありません。**メンテナが述べたのは「滅多に踏まない経路と頻繁に踏む経路で
扱いを変えるべき」までで、何がリリースのブロッカーになるか、既知の制限
として公開してよいか、頻度を測るべきかまでは定めていません。以下は
それを実務に落とすための一案で、個別の項目が反証すれば変わります。
...
この原則はメンテナが 0.2.4 の作業中に与えたものです。それ以前に書かれていた
...
**この対応関係の指摘自体はメンテナの発言ではなく、本プロジェクトの推論**
です。0.2.4 の作業中、`PUBLISHING.md` の条件だけを完成条件として参照して
いた誤りが見つかったため書かれました。両者が別の軸であることは 0.2 節の
定義から従いますが、「必要条件であって十分条件ではない」という定式化に
メンテナの裁定があるわけではありません。
