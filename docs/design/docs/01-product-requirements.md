...
**この 0 節は絶対的な信頼文書です。** 内容がメンテナの発言としても意図と
しても正しいことを、2026-08-18 に本人が確認しています。**リポジトリ内の
他のすべての規則・記録は、この節を履行するために積まれたもの**であり、
この節と衝突したときはこの節が勝ちます。その扱いは 0.6 に定めます。
...
判断の拠り所は次の二つで、これ以外に絶対のものはありません。

**軸A: 一般的な安全保障。** 個人情報の流出、拡張機能自身の情報の流出、
そして LSP 利用者の環境の安全性の担保。**これは LSP の構想とは別軸**で、
製品としてリリースする以上、責任を持って守らなければならない範囲です。
要するに、**常識的にやらなければならないことは、この 0 節に書いてあるか
どうかに関わらず行う**。0 節に根拠を探して見つからないことは、やらない
理由になりません。

**軸B: 0 節が定める LSP 本来の構想。** 0.1 の開発理由、0.2 の完成の定義、
0.3 のスコープ、0.4 の精度と出荷の原則。

**この二軸さえ担保されていれば、メンテナが個々の判断に介入する必要はない。**
これはメンテナ自身の言葉です。patch がメンテナの明示的な許可なく公開して
よいのも同じ理由で、**二軸を守るうえでメンテナのレビュー待ちがボトルネック
になってほしくない**から。patch に限定されているのは、「後回しでよい箇所を
必要以上に深掘りして、0.4 に書いた問題（1.0.0 が際限なく遠のく）を起こして
ほしくない」ためです。

ただし**これは自己判断を好きに行ってよいという意味ではありません。**
暴走を避けてほしい、というのがメンテナの明示的な要望です。二軸は判断の
基準であって、報告や可逆性を省いてよい免罪符ではない。

### 0.6 この 0 節の位置づけと、派生規則の扱い

**0 節は絶対的な信頼文書です。** ここに書かれていることは、メンテナの発言
内容としても意図としても正しいと確認されています（2026-08-18）。

**0 節以外のすべては、0 節を履行するために積まれた指示群です。** これには
`CLAUDE.md`、`AGENTS.md`、各 `NNN-*.md`、レジスタ、チェックリストが含まれ
ます。そして重要な前提として、**メンテナ自身も常に完璧な指示を出せるわけ
ではありません。** 過去のメンテナの指示にも、方向性がずれているもの、採択
したつもりで落ちたものがあり得ます。

したがって 0 節以外の規則・記録は、次の二点で検査対象です。

1. **本当に 0 節（および軸A）と同じ方向を向いているか。**
2. **0 節から派生した規則を守ろうとするあまり生まれた、ずれた指標では
   ないか。** 規則が規則自身を目的化していないか。

**規則と 0 節が衝突したときは 0 節が勝ちます。** 例えば、精度を上げるための
手続きが 1.0.0 を際限なく遠ざけているなら、その手続きは 0.4 に反しており、
手続きの方を変える。0.4 を守るために手続きを厚くしたはずが逆に働く、という
形がこの節の想定する典型です。

#### 文書に規則を書くときの判断指標

過去、指示から導出した規則がメンテナの声で書かれ、後のセッションが導出物の
方を指示として守る、という事故が実際に起きました（2026-08-18 の監査で 4 件
検出）。同じ過ちを繰り返さないために、規範的な文を書くときは次を守ります。

- **出所を明示する。** その文が (a) 0 節、(b) メンテナの実際の発言（日付
  つき）、(c) 本プロジェクトの推論・運用規則、のどれなのかを書く。(c) を
  (b) の顔で書かない。
- **二軸のどちらに資するかを言えないなら、それは規則ではなく好みです。**
  書くとしても規則としては書かない。
- **一般化するときは、一般化した事実そのものを書く。** 「メンテナは X と
  言った。これを一般化して Y とする」と書けば、後から Y だけを検証できる。
  Y だけを書くと検証不能になる。
- **規則を守ることが 0 節に反し始めたら、規則の側を疑う。** 規則は 0 節に
  仕える道具であって、逆ではない。

### 0.7 `docs/PUBLISHING.md` の 1.0.0 条件との関係
