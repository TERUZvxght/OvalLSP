...
**この数は 0.2.17 で 31 から 30 に変わりましたが、ロックが減ったわけでは
ありません。** 再導出が `Mutex.new` という*文字列*を数えていたため、RBS が
型名をどう表示するかを説明したコメントが 1 箇所、ロックとして数えられて
いました。数え直しは Prism で構文木を歩くようになり、コメントはノードでは
ないので数に入りません — `024.186`。印が付いていて緑だったのは、導出側と
散文が「ロックとは何か」について食い違っていたからで、この節が自分自身に
ついて書いている失敗の形そのものです。
...
発行する採番カウンタが、上の 31 箇所のうちの1つ（`TextDocument` のクラス
mutex)であり、他のどのロックの内側でも外側でもありません — 整数を1つ進める
だけで、その間に他を取ることがないためです。

**解析は「落ち着いた状態」に従います。** `didChange` は「この uri は解析が
必要」と記録するだけで、解析そのものは**読むべき入力が他に無くなったとき**に
走ります(037 の C9)。デバウンスではありません — 調整すべき間隔も待機
スレッドもなく、聞いているのは「入力がまだあるか」の一問だけです。答えられ
ない reader は「無い」と答え、その場合は従来どおり即座に解析します。
`@pending_analysis`(uri の集合)と `@settled_analysis_mutex` が上の 31 箇所に
含まれ、この mutex は他のどのロックの内側でも外側でもありません。
