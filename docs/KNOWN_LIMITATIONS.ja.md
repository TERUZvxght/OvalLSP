...
**公開している拡張機能が要求するのは Ruby 3.4.x** で、それ以外は拒否します。
VSIX はビルド時の `major.minor` 向けにコンパイルされた Prism と RBS の
ネイティブ拡張を同梱しており、`vscode/src/platformCompatibility.ts` が
検出した Ruby と突き合わせます。3.3 の環境では、機能が縮退するのではなく
プラットフォーム不一致のメッセージが出ます。実際に検証しているパッチ
バージョンは 3.4.5 と 3.4.7 です。

**Core のソース自体は 3.3 でも動きます。** CI は Core のスイート全体を
3.3 と 3.4 の両方で実行し、`core/ovallsp.gemspec` は `>= 3.3` を許します。
これはチェックアウトから Core を動かす場合に意味を持つ話で、公開している
VSIX が受け付ける範囲とは別の問いです。
