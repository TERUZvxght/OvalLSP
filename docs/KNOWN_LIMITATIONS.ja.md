...
## シンボリックリンク経由で開いたワークスペースで、全ファイルが二重に出ること

開いたフォルダがシンボリックリンクの場合（macOS の `/tmp` 配下、git の
worktree、ボリュームを指す `~/src` など）、この拡張機能は**解決後のパス**で
ファイルを解析する一方、エディタはリンク側のパスで会話します。「問題」パネル
には同じファイルが二度並び、解決後パス側の方には既に存在しない行のエラーが
残ります。直しても、保存しても、タブを閉じても消えません — そのパスへ再び
publish するものが無いためです。定義ジャンプも解決後パスを返すので、辿ると
同じファイルの2つ目のタブが開きます。 <!-- documents: 024.98 -->
