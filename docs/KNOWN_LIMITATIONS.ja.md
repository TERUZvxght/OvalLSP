...
現時点で事実でないことを言っている箇所です。前2つは診断として、3つ目は色
としてです。いずれも記録済みで、0.2.0では修正しておらず、しかもごく普通の
コードで目に見えます。自分で見つけてもらうのではなく、ここに挙げておきます。

- **クラスボディのマクロが未定義メソッドとして報告されます**(024.23)。
  `private`・`attr_reader`・`attr_accessor`・`private_constant`・
  `alias_method`・`include` は `Module` のメソッドですが、シングルトンの
  継承鎖は `Module` をモデル化していません。祖先がそれ以外は完全に既知で
  あるワークスペースのクラスでは、これらが1つ残らず報告されます。
  このリポジトリ自身の `core/lib` で計測すると、未定義メソッドの報告
  62件のうち**49件**がこの形です(`private` 34・`private_constant` 10・
  `attr_reader` 5)。ActiveSupport 8.1.3では776件のうち約211件がこの形で、
  `private`(72)・`alias_method`(56)・`attr_reader`(40)・`include`(26)が
  上位です。現在このエンジンが出す誤報告の最大の発生源ですが、唯一の
  発生源ではありません。どちらの残差も、`attr_reader` 自身が生成した
  リーダーへの呼び出し、`def self.` のボディ内のレシーバなしの呼び出し
  (同じモデル化漏れを裏側から見たもの)、そして後述の引数型の
  フォールバック(024.19)の混在です。
- **ルートが1つも読み込まれていないとき、`*_path`/`*_url` の呼び出しが
  すべて「存在しないルート」として報告されます**(024.24)。信頼されて
  いないワークスペース、およびRailsでないプロジェクトがこれに当たります。
  空のルート表が「知らない」ではなく「そんなルートはない」と答えるため
  です。計測では Ruby 3.4.7 の標準ライブラリで48件、prism 1.9.0で12件、
  すべて誤りでした(`original_path` や `dsl_path` はbundlerのごく普通の
  privateメソッドです)。Railsプロジェクトで Workspace Trust を拒否した
  場合にルートヘルパーが軒並み下線になるのは、これが理由です。
