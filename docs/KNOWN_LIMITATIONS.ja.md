...
Ruby **3.3 と 3.4** に対応します。`core/ovallsp.gemspec` の
`required_ruby_version >= 3.3` がインストールを許す範囲そのものです。
CIはCoreのスイート全体を両方で実行し、開発とリリースビルドは 3.4
(3.4.5、3.4.7)です。Ruby 3.5.x は、このプロジェクトのテストスイートで
実際に検証されるまでunsupportedとして扱います。

この「テストスイートで実際に検証されたら対応」という基準はこの節が以前から
掲げていたものですが、0.2.0のラウンド22まで逆に適用されていました。スイートが
走っていたのは 3.3 だけ — この文書が unsupported と書いていたバージョン —
であり、supported と書いていた2つはCIのどこでも走っていませんでした。主張を
狭めるのではなく、マトリクスを広げています。
...
前回のリリースから2つ無くなりました。**ルートが1つも読み込まれていないときに
`*_path`/`*_url` の呼び出しを「存在しないルート」として報告することは、
なくなりました**(024.24)。信頼されていないワークスペースとRailsでない
...
- **`def Foo.bar` はインスタンスメソッドとして記録されます。** そのため
  ...
  コーパスに残る引数個数の誤報**17件中9件**がこの形です。`net/http.rb` の
  `HTTP.get_response` などが該当します(024.32)。
...
- **ループで定義したメソッドは未定義として報告されます。**
  ...
  その呼び出しはすべて報告されます。計測したどのコーパスでも、このエンジンが
  出す誤報の最大の集中箇所です。**1ファイルで525件**(prism の
  `translation/ripper.rb`)。計算された名前の `define_method` も同じ形です。
  「静的解析の限界」ではなくここに挙げているのは、このエンジンの基準が
  「誤った報告は見逃しより悪い」であり、これが見逃しではなく誤報だからです。
  0.2.0 が変えるのは影響範囲です。開いていないファイルにも診断を出すように
  なったため、見つけに行かなくても Problems パネルに出ます。
