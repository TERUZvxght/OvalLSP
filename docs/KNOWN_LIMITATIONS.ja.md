...
答える前に停止はしません。0.2.2 まで、この文書も両方の README も導入ページも
「停止する」と書いていました。
...
**0.2.2 で変わったのは支払う回数であって、1回の金額ではありません。**
診断は 300ms の静止を待つようになり、待っている間に届いたバージョンは
待っているものを置き換えます。つまり連続して打っている間の再解析は1回で
済み、打ち続けている限り0回です。それでも支払うのは「手を止めたとき」です
— そのサイズのファイルで一瞬手を止めれば、ホバーも補完もシグネチャヘルプも
その数秒の後ろに並びます。上の数字は解析1回あたりのもので、0.2.2 より前は
1打鍵あたりでもありました。

設計文書は解析そのものに 300ms と書いているので、これは好みの問題ではなく、
