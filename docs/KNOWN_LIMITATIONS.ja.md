...
実測で72箇所、この拡張機能は失敗を捕まえて、本物の答えに見える値(空のリスト、
`nil`、`false`)でそのまま続行します。そこに当たったときはエラーではなく
「もっともらしい答え」が返るため、ホバーが出ない・検査が黙っているのが、
この解析エンジンの限界ではなく**報告されなかった失敗**である可能性があります。
確認先は Output チャンネルですが、常に行が出るとは限りません。 <!-- documents: 024.122 -->
