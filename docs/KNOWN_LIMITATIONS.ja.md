...
- 答えの無い位置でのホバーが、何も返さないのではなく空のホバーを返します。 <!-- documents: 024.127 -->
...
- `b = nil; b ||= "x"` のあとに `b` をホバーすると **`nil`** と出ます。
  その位置の `b` は `String` です。これは「答えが無い」ではなく「間違った
  答え」であり、本プロジェクトは後者をより重い欠陥として扱います。 <!-- documents: 024.131 -->
- concern の `included do` の中で定義した scope は型を持ちません。 <!-- documents: 024.132 -->
- キーワード専用メソッドに位置引数を渡すと「0 個の引数を取る」と表示され
  ます。 <!-- documents: 024.133 -->
