...
Output チャンネルに出します。できなければ `gem install prism rbs` を促す
エラーです。Ruby 3.3 はどちらも同梱しているので、たいていは前者になります
...
次の行が `value`・`return 1`・`other_thing(1)` でも同じです。名前を打ち
終えれば消えます。

**これは OvalLSP の推測ではなく、Ruby 自身がそう読むということです。**
行末の `.` は行継続です。上のコードを `ruby -c` は `Syntax OK` と言い、
実行すれば `undefined method 'b=' for an instance of Article` が上がり、
`b=` セッターが実在すれば本当に呼ばれます。OvalLSP は、処理系が指すのと
同じ行で、同じメソッド名を報告しています。誤読しているものは何もなく、
打ち終えるまでの間だけテキストが別の意味を持つ、というだけです。

ですから直し方は「検査を足す」ではありません — テキストの中に検査できる
ものが無く、「レシーバとメソッドが別の行にある」という規則を入れれば、
意図して書かれる行末ドットのチェーン記法まで消えてしまいます。0.2.3 は
報告を打ち終えるまで遅らせようとし、そうすれば打っている最中は出なくなる
はずでしたが、その変更は別の理由で巻き戻しました(024.57)。完全に無くすには
編集位置が必要で、カーソルのある行についての報告を保留することになります — その行の本物のエラーも隠れるため、0.3.0 までに直す欠陥
ではなく、意図的な調整として
[0.4.0](https://github.com/TERUZvxght/OvalLSP/blob/main/docs/ROADMAP.ja.md)
に置いています(024.41)。 <!-- documents: 024.41 -->
...
## VSIX がビルドされていない Ruby で起きること

0.2.1 から、拡張機能はその Ruby が `prism` と `rbs` を自前で持っているかを
確認し、持っていればそちらで動きます。拒否ではなく、未検証の組み合わせで
動くということです。トーストではなく Output チャンネルの1行として出ます。
`OvalLSP: Show Version Information` でも同じ内容を確認できます。

(0.2.1 では、最初の変更と一緒に直さなかった2つ目のバージョン検査が赤い
エラートーストを出したままで、Marketplace の説明文も逆のことを書いていました。
どちらも 0.2.3 で修正しました。)

## コアクラスと同じ名前を持つ自作クラス

**そのクラスのリテラルが、自分のメソッドを見つけられなくなります。**
ワークスペースに `Serializer::Elements::String` があると(入れ子の深さは
問いません)、`"hello".` はそのクラスのメソッドを出し、Rubyのメソッドは
1つも出しません。`upcase` も `strip` もありません。ホバーと定義へ移動も
同じ置き換えに従います。自作の `Set`・`Time`・`File`・`Range` でも同様です。

そこに診断は出ません。0.2.1 で「置き換えたレシーバについて報告しない」ように
したためです。つまり間違いとして印は付かず、答えが別のクラスについてのものに
なるだけです。自分の名前空間の中から自作クラスを参照する場合は正しく解決します
— この段落は以前、そちらが壊れていると誤って書いていました(024.47)。 <!-- documents: 024.47 -->
...
**まれで、大きなファイルに限ります。** Runtime Agent がルートやモデルを
供給したとき、あるいは準備完了になったとき、開いているファイルはすべて
背後のスレッドで再解析されます。このパスは「どのファイルを解析するか」を
走り始める前に決めており、走っている最中には見直しません。
...
**数千行のファイルでは数秒です。** 署名の初回読み込みを除いた計測:
`net/http.rb`(2,574行)で 2.1 秒、`rubygems/specification.rb`
(2,666行)で 5.3 秒。しかもファイルの大きさより速く増えます。Coreは要求を
1つずつ処理するので、ホバーも補完もシグネチャヘルプもその後ろで待ちます。

**0.2.3 はここを変えようとして、その試みを巻き戻しました。**
報告を「打ち終えるまで」遅らせれば連続入力は1回の解析にまとまり、実際に
そう計測できました。ただし同時に2つの競合を生み、1つのファイルに対して
同時に走る解析の数を抑えられませんでした。4回のレビューラウンドがそれぞれ
別の欠陥を見つけています。詳細は `024.57` に記録してあり、設計を変えて
再度試みます。現時点のコストは従来どおり、1打鍵あたりです。

設計文書は解析に 300ms と書いているので、これは好みの問題ではなく、
1桁以上外している要件です。0.2.1 までここに記載がありませんでした
(024.45)。数百行のファイル — アプリケーションコードの大半 — は 0.1 秒を
大きく下回ります。 <!-- documents: 024.45 -->

## 診断は今も1打鍵ごとに届きます

**デバウンスはしていません。0.2.3 でそうしようとして、やめました。**
ファイルの報告を「打ち終えるまで」遅らせれば連続入力は1回の解析にまとまり、
実際にそうなることは計測できました。ただし同じ変更が、パネルを消した後に
報告が届く経路を2つ生み、1つのファイルに対して同時に走る解析の数を
抑えられませんでした。0.4秒間隔の12回の編集で12回の解析が始まり、うち5回が
同時に走り、それぞれがホバーと補完の必要とするロックを保持します。4回の
レビューラウンドがそれぞれ別の欠陥を見つけたため、出荷せずに取り除きました。

これによる負担は上の
[編集の再解析にかかる時間](#編集の再解析にかかる時間)のとおりです。数千行の
ファイルでは1打鍵ごとにかかります。発行するかどうかを各発行者が個別に決める
のではなく、1つの書き手が決める設計にして再度試みます(024.57)。 <!-- documents: 024.57 -->
...
## CRuby以外のエンジンでエラー通知が2つ出る

**`ovallsp.rubyExecutablePath`をJRubyやTruffleRubyへ向けると、ウィンドウ
ごとに赤い通知が1つではなく2つ出ます。**どちらも同じ事実 — 同梱の
ネイティブペイロードはCRuby向けにビルドされており、そのエンジンには
適用できない — を報告しており、1つはCore起動前のチェックから、もう1つは
その後の拡張機能/Core間ハンドシェイクから出ています。前者の文面は
`gem install prism rbs`を勧めますが、エンジンが不一致である場合これは
問題ではなく、すでに実行済みの助言かもしれません。拡張機能は起動し、
Coreも動作します。重複しているのは報告だけです(024.65)。 <!-- documents: 024.65 -->
