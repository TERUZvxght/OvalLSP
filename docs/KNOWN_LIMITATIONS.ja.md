...
**知らせて、そのまま続きます。** 拡張機能と Core がバージョン・プロトコル・
ビルド識別子・ペイロードのハッシュのいずれかで食い違うと、エラー通知が出て
詳細が Output チャンネルに出ます — そしてセッションはそのまま動き続けます。
答える前に停止はしません。0.2.3 まで、両方の README と両方の導入ページが
「停止する」と書いていました。
...
`OvalLSP: Show Version Information` で確認できます(024.55)。 <!-- documents: 024.55 -->
...
## 自分の別のクラスと名前を共有しているクラス

ワークスペース内の2つのクラスが、別々の名前空間で短い名前を共有している場合
（ActiveRecord の `Comment` と並ぶ `Billing::Comment`、トップレベルのものと並ぶ
`App::Config` など）、自分の名前空間の内側から修飾なしで参照すると、もう一方で
答えられることがあります。しかも両方向とも誤りです — 動く呼び出しが「存在しない
メソッド」と報告され、実際に失敗する呼び出しは報告されません。補完はもう一方の
クラスのメソッドを出します。

どちらが勝つかは「近い方」ではありません。名前空間を書けば（`Billing::Comment.new`）
解消します。 <!-- documents: 024.103 -->

## concern の `class_methods do` が何に付くか

**クラスではなくインスタンスに付きます。** `class_methods do ... end` で書いた
concern はクラス側にメソッドを宣言しますが、この拡張機能はインスタンス側に
帰属させます。`Article.new.` の補完に出て、ホバーも定義ジャンプもそこで答え、
未定義メソッド検査は実際には失敗する呼び出しを受理します。古い
`module ClassMethods` の形は正しく扱われます。 <!-- documents: 024.104 -->

## `class << self` の中の `private`

**候補一覧に影響しません。** `class << self` の中で private にしたメソッドや
`private_class_method` を付けたメソッドは、呼ぶと失敗するにもかかわらず、補完に
出て未定義メソッド検査にも受理されます。`private` の他の置き方 — クラス本体、
`private def x`、`private :x`、concern の `included do` の中、入れ子クラスの前 —
はいずれも正しく動きます。 <!-- documents: 024.105 -->

## `module_function` と `extend self`

**何も出ません。** `module_function` や `extend self` でモジュール側に生えた
メソッドは、補完に一切現れません。同じモジュール内の `def self.x` や
`class << self` は動きます。モジュールのクラスレベル呼び出しも検査されないので、
そこでのタイプミスはクラスの場合と違って報告されません。 <!-- documents: 024.106 -->

## 補完の中の alias

**出ません。** `alias` と `alias_method` はホバー・定義ジャンプ・未定義メソッド
検査には理解されており、補完だけが知りません。メソッドに別名を付けた後に `a.` と
打つと、その別名が存在しないように見えます。 <!-- documents: 024.107 -->

## 補完の中の protected メソッド

クラスの外から明示的なレシーバに対して出ますが、そこで呼ぶと失敗します。private
なインスタンスメソッドは同じ位置で正しく除外されています。 <!-- documents: 024.108 -->
...
**1打鍵ごとに全体の再解析が走り、その都度 publish します。** 1打鍵あたりの
実測で、1000行 53ms、2000行 155ms、4000行 368ms。おおよそ 2000 行を超えると
打鍵の間隔より長くなるため、作業が入力に追いつかなくなります。

見え方としては、存在するメソッド名を打っている最中、その途中までの各文字列に
赤い波線が出続け、打ち終えてから6秒ほど遅れてパネルが綺麗になります。ホバーと
補完もその作業の後ろに並びます — 4000行のファイルで素早く10打鍵した後の
ホバーが 3.4 秒、20,000行では 25 秒かかり、その間もう一つ開いていたファイルにも
診断が出ませんでした。 <!-- documents: 024.101 -->
