...
## リリースが割り当てられていない既知の制限があります

ここに挙げた制限のうち18件は、未解決の欠陥として記録されていますが、
どのリリースで直すかが決まっていません。直さないという判断ではなく、
いつ直すかをまだ誰も決めていない、という意味です。これは日付の不在で
暗黙に示すのではなく、スケジューリング側の欠陥として記録しています
(`024.153`)。 <!-- documents: 024.153 -->

## パスヘルパーのoptionalセグメントが少なく報告される

ルートヘルパーのSignature Helpは、必須パラメータと、optionalのうち
`format` だけを表示します。Runtime Agentがoptionalセグメントを検出する
方法がパス文字列に `(.:format)` が含まれるかどうかの判定であるため、
`get "/posts(/:page)"` のようなルートはoptionalパラメータなしとして扱わ
れ、`page` を欠いたシグネチャが完全なものとして表示されます。必須パラ
メータはRails自身から読んでおり、正しく表示されます。 <!-- documents: 024.136 -->
