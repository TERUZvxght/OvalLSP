...
正しく何も言いませんが、定義している行の方は報告されます。

0.2.11 はこれを黙らせる修正を入れて、同じリリース内で取り下げました。その修正は、
どこか1ファイルが `Module`・`Object`・`Kernel` を再オープンしているだけで、
ワークスペース全体の `Foo.bar` 検査も黙らせてしまうものでした。 <!-- documents: 024.110 -->

## 閉じずに開き直したファイル

エディタが `didClose` を送らずに同じファイルへ2回目の `didOpen` を送り、新しい
バッファの版数が前より小さい番号から始まった場合、そのファイルの診断は版数が
元の位置を追い越すまで届かなくなります。VS Code は先に `didClose` を送るため、
これには変わったクライアントが必要です。 <!-- documents: 024.118 -->
...
## Linux でマイグレーションファイルを新規作成しても更新されないことがあります

`db/migrate/` の下にファイルを追加しても、Linux ではファイル監視イベントが
発生しません。そのためこの拡張機能はスキーマが変わった可能性を知らされず、
モデルについて知っている内容を更新しません。既存のマイグレーションの編集、
および他の監視対象ファイル(`db/structure.sql`・`.rb`・`.rbs`・`.rbi`・
`Gemfile.lock`)は正常に動作します。ウィンドウを再読み込みすれば反映されます。
macOS では再現しません。 <!-- documents: 024.120 -->
