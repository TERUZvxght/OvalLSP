...
自作クラスまで黙らせるのです。さらに、こうしたクラスがワークスペースに
ある間はリテラル側にも代償があります。`"hello".` の補完は String ではなく
そのクラスのメンバーを出し、`"hello".upcase` のホバーはコアクラスから
答えます —
読み手どうしが食い違っており、どちらが正しいかはエンジンの持たない情報に
依存します(024.47)。トップレベルにあるコアクラスと同名のクラスは影響を
