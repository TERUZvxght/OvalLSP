...
意図して書かれる行末ドットのチェーン記法まで消えてしまいます。0.2.2 で
診断は打ち終えてから300ms待つようになったので、打っている最中は出ません。
完全に無くすには編集位置が必要で、カーソルのある行についての報告を保留する
ことになります — その行の本物のエラーも隠れるため、0.3.0 までに直す欠陥
ではなく、意図的な調整として
...
## 一度消えたルートの報告が戻ってくることがある

**まれで、大きなファイルに限ります。** Runtime Agent がルートを供給すると、
開いているファイルはすべて再解析され、ルート無しで出ていた `*_path` の報告は
消えます。ルートが届いた瞬間に、解析へ数秒かかる大きさのファイルで手を止めて
いた場合、すでに走っていた解析が最後に終わり、ルート以前の結果をその上に
書きます。次の編集で消えます(024.56)。 <!-- documents: 024.56 -->
...
同時に走る解析の数を抑えられませんでした。3回のレビューラウンドがそれぞれ
