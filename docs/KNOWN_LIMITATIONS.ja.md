...
- **何も代入されない `@ivar` の読み出し**はERBビューだけが対象で、
  代入の集合を完全に列挙できる場合に限ります。コントローラの継承鎖が
  すべて読めること、`instance_variable_set` を使っていないこと、
  モジュールをmix-inしていないこと、解析がモデル化していないコールバック
  形式がないこと。どれか1つでも該当すれば、そのビューでは検査自体が
  黙ります。兄弟アクションでの代入も、意図的に黙らせます。
- **開いていないファイルの診断**はプロセス内では動きますが、実物のRails
  アプリに対するE2E検証がありません。そのために書いた例が45秒経っても
