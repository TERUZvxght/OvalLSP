...
**名前空間の中にあると解決しなくなります。** `Billing::Range`・
`Admin::File`・`Reporting::Time` を、Rubyが自分の名前空間の中からクラスを
参照する普通の書き方 — 修飾なしの名前 — で書くと、0.2.1 ではホバーも定義へ
移動も補完も答えません。0.2.0 は答えていました。Ruby自身が宣言している
修飾なしの名前を、ワークスペースのクラスに答えさせない規則があるためです。
リテラルが生む `String` に対しては正しく、あなたが書いた `Range` に対しては
誤りです(024.47)。トップレベルにあるコアクラスと同名のクラスは影響を
受けません。 <!-- documents: 024.47 -->
