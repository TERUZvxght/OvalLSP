...
## 消したはずの診断や、閉じたファイルの診断が残ることがある

**まれで、この再解析パスが走っている間に限ります — その長さは、1つの
ファイルの大きさだけでなく、開いているファイルの数に応じて伸びます。**
Runtime Agent がルートやモデルを供給したとき、あるいは準備完了になった
とき、開いているファイルはすべて背後のスレッドで再解析されます。この
パスは「どのファイルを解析するか」を走り始める前に決めており、走って
いる最中には見直しません。

そこから2つ起きます。解析へ数秒かかる大きさのファイルで手を止めていた場合、
ルート無しで出ていた `*_path` の報告が、訂正後の結果の上に書き戻されること
があります(次の編集で消えます)。そして**そのパスが別のファイルを解析して
いる最中にファイルを閉じると、そのファイルのエラーが「問題」パネルに残り
ます** — 誰も開いていないファイルを再発行するものが無いため、セッションの
間ずっとです。開き直せば消えます(024.56)。 <!-- documents: 024.56 -->
...
## コアクラスが意図された場所で、補完があなたのクラスのメソッドを出すこと

あなたのクラスが、入れ子のコアクラスと名前を共有している場合 — 自作の `Stat` と
`File::Stat` など — ホバーと診断は正しく答えるようになりましたが、**補完は今も
あなたのクラスのメソッドを出します**。型はどちらの場合も正しく、メンバー一覧の
引き当てだけが修飾なしの名前で行われています。

0.2.5 は前者2つを直し、3つ目を直していません。名前を共有するクラスが無い場合、
`File.stat(path).` の補完は「何も出ない」から「正しい 167 件」に改善している
ので、新たな不具合ではなく途中で止まった改善です。 <!-- documents: 024.78 -->

## `Model.first` の補完が返すもの

**何も返しません。** `Order.first.` は補完候補を出さず、`Order.recent.first.`
はモデルの全候補を出します。より一般的な書き方の方が動きません。 <!-- documents: 024.79 -->
