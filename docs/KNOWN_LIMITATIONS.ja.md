...
動くということです。これについて、まだ2つ間違っている点があります。

- **赤いエラートーストは今も出ます。** 最初の変更と一緒に直さなかった
  2つ目のバージョン検査が「Core がバージョン互換ではない」と言います
  (024.49)。OvalLSP は動いています。トーストが説明しているのは同梱物の
  事情であって、あなたの状況ではありません。 <!-- documents: 024.49 -->
- **Marketplace の説明文は逆のことを書いています** — そういう組み合わせで
  OvalLSP は「黙って機能を落としたり中途半端に起動したりしない」と
  (024.50)。いまは意図的にそうしており、その文面が追いついていません。 <!-- documents: 024.50 -->

## 0.2.1 にアップグレードした後の初回起動

**何かが答え始めるまで時間がかかることがあります。** 0.2.1 はキャッシュの鍵に
ビルドのバージョンを入れたため、前のバージョンが書いたディレクトリがすべて
放棄されます。それを掃除する処理が、要求に answers するのと同じスレッドで
走ります。長く溜まったキャッシュ — 28,643ディレクトリ・2.8GB という実測が
あります — では30秒程度、1度だけかかります(024.51)。2回目以降の起動は
影響を受けません。 <!-- documents: 024.51 -->
