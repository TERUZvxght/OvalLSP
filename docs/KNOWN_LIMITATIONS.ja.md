...
- **引数の個数**は検査しますが、これまでに計測したコードに対しては1件も
  ...
  **109件**出しており、そのすべてが正しく動く Ruby でした。下の「引数の型」
  ...
  報告されていました。gemのコーパスはこの検査にとって最悪に近い条件です —
  依存先が存在しないため名前が置き換えで解決されます — 実アプリケーションでの
  精度はいまも未計測です(024.40)。 <!-- documents: 024.40 -->
...
正しく働き、0.2.13 以降はリテラルを回すブロック(`[1].each { private }`、
`%w[a b].each { module_function }`)でも正しく働きます。
