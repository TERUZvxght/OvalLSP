...
- **`def Foo.bar` はインスタンスメソッドとして記録されます。** そのため
  ...
  `HTTP.get_response` などが該当します(024.32)。 <!-- documents: 024.32 -->
...
- **引数の個数**は検査しますが、これまでに計測したコードに対して出す報告は
  すべて誤りです。Rubyの標準ライブラリ・Railsの5つのgem・minitest に対して
  **15件**、いずれも正しく動くRubyです。10件は `def Const.method` が
...
- **開いていないファイルの診断**は1回のパスで2,000ファイルまでで止まります。
  ...
  実物のRailsアプリでは、一度も開いていないファイルがプロセス起動から1.4秒で
  answered されます。G17 の行と例が現在あり、元の計測が何を踏んだと考えられるかは
  024.14 に記録しています。)
