...
**0.2.7 で解消しました。** ファイルを閉じても、そのエラーが「問題」パネルに
セッションの間ずっと残ることがありました。背後で走っている再解析パスは
「どのファイルを解析するか」を走り始める前に決めているため、既に始まっていた
パスが、閉じたばかりのファイルを再発行していたためです。誰も開いていない
ファイルを再発行するものは無いので、開き直すまで残りました。現在、publish は
版数を持つことと、そのバッファがまだ開いていることの両方を要求し、ファイルを
閉じる操作は常に勝ちます。
