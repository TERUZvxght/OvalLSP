...
そのうち 53 件が誤りでした。** 残る 1 件は議論の余地があります — 呼び出し元の
クラスには本当に定義されていない abstract template method で、それでも Ruby
開発者は報告してほしくない類のものです。
この検査は「見逃しより誤報の方が悪い」という方針の上に作られていますが、この
証拠に照らすとその方針を満たせていません。

アプリケーションコードにとって重要な原因は、**同一ファイル内で定義された
モジュールを、入れ子の名前空間の内側から `include` している**クラスが、その
モジュールのメソッドを失い、検査が「存在しない」と報告することです。Rails の
concern がまさにこの形です。`Rack::Request` が実例で、`Helpers` を include して
いるのに `request_method` が無いと言われます。

残りはメタプログラミングで定義されるアクセサ（`attr_atomic` 等。静的解析には
見えないので、報告ではなく沈黙すべきものです）と、あなたの Ruby では決して
実行されないプラットフォーム固有ファイルです。 <!-- documents: 024.76 -->

**存在しないメソッド呼び出しは、relation を経由すると見逃されます。**
`Order.recent.first.no_such_method` は何も報告されず、
`Order.find(id).no_such_method` は通常どおり報告されます。補完はその位置の型を
知っており（存在しない名前は候補に出しません）、情報はあるのに検査が使って
いません。`Model.scope.first` は日常的な書き方です。 <!-- documents: 024.77 -->
