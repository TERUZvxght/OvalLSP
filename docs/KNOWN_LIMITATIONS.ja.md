...
そのうち 53 件が誤りでした。0.2.6 でこれが 10 件になります。** 測定は毎回
...
- **別の Ruby 実装向けのファイル。** JRuby 専用のソースは `java` を呼びますが、
  あなたの MRI にはありません。10 件中 7 件。JRuby 固有のファイルを開かない
  限り目にしません。
- **サブクラスが与えるメソッド。** サブクラスが定義するメソッドを呼ぶ抽象
  クラス — 意図的な template method — は報告されます。そのクラス単体では
  実際に呼び出しが失敗するためです。10 件中 2 件。
- **実行時に決まるモジュール。** 変数やコンストラクタ引数に入ったモジュールを
  `extend` すると、この拡張機能が名前を特定できない祖先が残り、そこから来る
  はずのメソッドが「存在しない」と報告されます。10 件中 1 件。 <!-- documents: 024.76 -->
