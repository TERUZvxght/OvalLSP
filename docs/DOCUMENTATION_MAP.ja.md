...
| **前ラウンドと同じ箇所を指摘したレビューラウンド** | 機械的な対策 — 実装の共有、値を作る場所へのルールの移動、見えていなかった入力をガードに与えること。その1件に対する回帰テストは対策ではなく、3度目の手当ても対策ではない | 判断そのものは — 。機械的に決まる2点は `scripts/review_round.rb` が持つ: `start <method>` は前ラウンドと同じ method と、clean でないツリーでの開始を拒否し、`close` はラウンド中に index が動いていれば拒否する(`core/spec/meta/review_round_spec.rb`) |
| **レビューラウンドの開始と終了** | そのリリースの `docs/design/tasks/NNN-*.md`。`ruby scripts/review_round.rb start <method>` が見出しと空の指摘表をそこに書き、そのラウンドが読む index を `core/tmp/`(untracked)に記録する。`ruby scripts/review_round.rb close` は index が動いたラウンドを拒否する。リリースを越える指摘はその表ではなくレジスタへ | `core/spec/meta/review_round_spec.rb`、および指摘欄を持つタスク文書が指摘の行き先を明記していることを見る `core/spec/meta/task_findings_section_spec.rb` |
| **先送り項目**(`024.*`) | `docs/design/tasks/024-deferred-review-findings.md` の `yaml` メタデータブロック(`status`、解決したら `released-in`)。エントリの削除は、その番号を引用している箇所がツリーに1つも無くなってから——暦ではなく grep で判断する(同ファイルの凡例を参照)。**エントリを開くのも閉じるのも手編集ではなくコマンド**: `ruby scripts/issues.rb promote <n> …` が1件を書き、`ruby scripts/issues.rb close 024.N --released-in V` が1件を解決する。後者はどちらかの言語がまだその指摘の段落を公開している間は拒否し、`--drop-paragraphs` はマーカーではなく `##` 節ごと取り除く | `core/spec/meta/issues_tool_spec.rb` |
