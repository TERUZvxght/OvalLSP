...
**このツリーには入っていません。** `site/` は
`claude/github-pages-official-site-fef0f5` にあり、まだ取り込んでいません。
そのため引き金の表のサイト関連の行は、ここからは辿れません。辿るのではなく
*持ち越す*必要があり、2つ下の節の一覧がその持ち越し先です。変更によって
サイトが古くなる箇所は、ブランチが取り込まれるまでその一覧に書いてください。
辿れない引き金は、誰も辿りません。
...
- **`site/capabilities.html` と `site/ja/capabilities.html` が 0.2.0 の
  ...
  ドキュメント、セマンティックハイライトの6つです。`site/roadmap.html` にも
  `docs/ROADMAP.md` から消した 0.2.0 の節が残っています。このまま公開すると、
  プロジェクト自身のサイトが「出荷済みの機能6つはこれから」と言うことに
  なります。下のトップページの項目が順序に依存するのはこれが理由で、
  編集としては同じものです。
