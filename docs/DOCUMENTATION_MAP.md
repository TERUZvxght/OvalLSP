...
could notice.** The rows whose trigger is a file are enforced from
preflight; the rest are walked by hand, and before a release for
everything in it — `docs/RELEASE_CHECKLIST.md` says when. `AGENTS.md`'s
...
| **The release procedure** — anything in `vscode/scripts/release.sh`, `scripts/preflight.rb`, or the gates either invokes | `docs/PUBLISHING.md`, `docs/RELEASE_CHECKLIST.md` (the evidence column names the script, and a renamed or removed one leaves a row citing nothing), `docs/DEVELOPMENT.md` where it describes running it — which points at `--list` instead of enumerating (`024.195`) | `core/spec/meta/release_script_guard_spec.rb`, and `046`'s C6 for the narrower claim that every script an evidence column cites is invoked by something. **Neither fires on an edit to `release.sh` itself**, which is what `024.171` records: a fix applied where the procedure runs and not where a person is told what to run. **Enforced as `release-procedure`** by `scripts/check_doc_triggers.rb` |
| **A change reverted mid-release** — the change most likely to leave documentation behind, because the prose was correct when written and nothing about undoing the change announces that it undid the reason for a paragraph; grep the tree for the thing being reverted before committing the revert (see `docs/REVIEW_LOOP.md`'s same-place rule) | a `024.*` entry naming the root cause and the direction actually needed, both changelogs if a bullet was already written for it, and the document behind the `AGENTS.md` line the episode informs | — |
| **A review round finding the same place the previous round did** | a mechanical countermeasure — a shared implementation, a rule moved to where the value is produced, a guard given the input it could not see — *not* a regression test for the one instance, and not a third hand fix | — for the judgement, which no scanner makes |
| **A review round opened or closed** | the release's `docs/design/tasks/NNN-*.md`, through `ruby scripts/review_round.rb start <method>`: it writes the round's heading and an empty findings table there and records the index the round reads, under `core/tmp/` and untracked. `close` refuses a round the index moved under. Findings go in that table as the round produces them; anything outliving the release goes to `docs/ISSUES.md`'s intake, never into the task document alone | `core/spec/meta/review_round_spec.rb` (the method rule and the fixed-tree rule, each with the case that must still go through) and `core/spec/meta/task_findings_section_spec.rb` (a task document with a findings section says where findings actually go) |
| **A deferred finding** (`024.*`) | its `yaml` metadata block in `docs/design/tasks/024-deferred-review-findings.md` (**the register is two files since 0.3.0's `024.R9`** — open entries there, resolved ones in `024-deferred-review-findings-resolved.md`, and `DeferredFindings.register` is the only reader that knows: anything reading the live file directly silently loses three-quarters of the entries. Resolving an entry means moving it across, and the generated index in the live file carries both) — `status`, and `released-in` once it is resolved; delete the entry once nothing in the tree still cites it by number — grep first, do not go by the calendar (see that file's own legend). Entries go in numeric order and the index at the head is generated: run `ruby scripts/reindex_findings.rb` rather than editing it. **Opening and closing an entry are commands rather than hand edits**: `ruby scripts/issues.rb promote <n> …` writes one from an intake item, `ruby scripts/issues.rb close 024.N --released-in V` resolves one — and the second refuses while either language still publishes a paragraph for the finding, `--drop-paragraphs` taking the whole `##` section rather than the marker | `core/spec/meta/deferred_findings_spec.rb` (status/kind/note/both-language anchors, **and that the order and index are current**) and `core/spec/meta/issues_tool_spec.rb` (the write primitive, and each command's refusals with the case that must still go through) |
| **An issue nobody has triaged yet** (found in a review, a workflow, or by driving something) | the **Intake** section of [`docs/ISSUES.md`](ISSUES.md), which is where an issue starts. It moves into the register once it has been *driven* and its kind, target and user-visibility are known — `docs/ISSUES.md`'s own "The rule" says in what order. Do not open a register entry for something unverified: every field there is enforced, so an entry added early is an entry corrected later. `ruby scripts/issues.rb intake add` puts one here; `ruby scripts/issues.rb promote <n>` takes it out by its position in the list `ruby scripts/issues.rb intake` prints, and restates the list's own count | `core/spec/meta/issue_index_spec.rb` (the generated half is current with the register, and regenerating twice changes nothing) and `core/spec/meta/issues_tool_spec.rb` (both shapes of bullet read as one list, and the count restated when an item leaves) |
| **Install steps, prerequisites, or the extension id** | `README.md` + `.ja.md`, `docs/PUBLISHING.md`, `site/getting-started.html` + `site/ja/getting-started.html` | `core/spec/meta/extension_identity_spec.rb` (the id every install command and `itemName` link prints, against `vscode/package.json`, which is the only place it is true). The prose around it is not compared; the id is |
| **What the extension records, keeps, or writes to disk** | `vscode/PRIVACY.md` + `.ja.md` — the single source of truth for this; `site/security.html` + `site/ja/security.html`; and every place 0.1.12 found restating the list or the cache path, each of which must point at PRIVACY rather than copy it (this list has been short three times; add to it rather than trusting it): `docs/design/docs/12-release-and-support.md`, `docs/design/tasks/019-runtime-observation.md`, `019-runtime-observation-notes.md`, `021-persistent-cache-notes.md`, the cache paragraph in `vscode/README.md` + `.ja.md`, `docs/SECURITY_CHECKLIST.md`'s observation and cache-deserialisation sections, `core/lib/ovallsp/observation/store.rb`'s `#invalidate_changed` doc, `core/lib/ovallsp/observation/observed_signature.rb`'s `code_fingerprint` doc, and **both changelogs**, which restate the disk claim in prose and went stale against PRIVACY for a whole round because this row did not name them | `core/spec/meta/privacy_parity_spec.rb` (EN ⇔ JA: section count, cross-links, three named claims, and the length of the recorded-items list). **Enforced as `privacy`** by `scripts/check_doc_triggers.rb` |
| **Anything about the Runtime Agent, workspace trust, or what the extension executes** | `SECURITY.md` + `.ja.md`, `site/security.html` + `site/ja/security.html`, `docs/EXTENSION_CAPABILITIES.md`'s "does not promise" section | `core/spec/meta/security_parity_spec.rb` (EN ⇔ JA: section count, the reporting route both must name, and the cross-links). Structure and the acted-on facts only — the two are translated independently, so comparing prose would buy a stricter check by making the translation worse. **Enforced as `security-surface`** by `scripts/check_doc_triggers.rb` |
| **A known limitation** | `docs/KNOWN_LIMITATIONS.md` + `.ja.md`, and the site page that claims the opposite, if any | `core/spec/meta/deferred_findings_spec.rb` (an open `024.*` entry that is not a roadmap plan and declares `user-visible: yes` carries one `<!-- documents: 024.N -->` marker, at the end of the line that documents it, in each language; one declaring `no` states why). It read `kind: defect` only until 0.3.3, so seven open `friction` entries were never demanded -- `024.173`'s filter, in the reader that fix did not reach. **And when you remove a limitation, remove its heading with it**: the marker is inside the body, so it leaves with the body and this guard goes quiet. That happened three times before `scripts/check_bodyless_headings.rb` (`core/spec/meta/bodyless_headings_spec.rb`, and a preflight gate) started reading the shape instead |
| **Which Ruby, Rails or platform the product accepts** — including anything in `vscode/src/platformCompatibility.ts`, `versionInfo.ts` or `rubyResolver.ts` | `docs/SUPPORT_MATRIX.md` + `.ja.md` (every affected row, not only the one you came for), `docs/KNOWN_LIMITATIONS.md` + `.ja.md`, **`vscode/README.md` + `.ja.md` — the Marketplace description, which states this in prose *and* in an environment table**, `site/getting-started.html` + `site/ja/`, the platform callout on `site/index.html` + `site/ja/index.html` (named here in 0.2.3's merge round, after the honesty pass missed it the same way this table once missed the badge), both changelogs, `core/ovallsp.gemspec`'s `required_ruby_version` if the floor moved | `core/spec/meta/supported_versions_spec.rb` (the set of Ruby patch releases named as tested, across both support matrices and both Marketplace READMEs, and that each names a minor `ci.yml` runs). `SUPPORT_MATRIX` named three and the READMEs two for a whole release before 0.3.1 noticed. **Enforced as `support-surface`** by `scripts/check_doc_triggers.rb` |
| **Where a release's work lives** (a branch created, renamed, renumbered, or work moved between branches) | the release's `NNN-*.md` on `main` names the branch; `AGENTS.md`'s "Branches, commits, releases"; `docs/DEVELOPMENT.md`'s "Branches and pull requests" — 0.2.3 was prepared twice in parallel because this row did not exist (028). **The convention is one branch per version merged by pull request** (`release/<version>`), set 2026-09-01; a change to it lands in all three | `scripts/check_release_pointers.rb` and `core/spec/meta/release_pointers_spec.rb`, added in 0.3.1: a `release/x.y.z` branch git can see that no task document names is 028's shape, and `release/0.3.1` was exactly that when the check was written. It reports rather than asserts when it can see no branches, because a tarball legitimately has none |
...
| **A behaviour relied on from outside this tree** — the client, VS Code, the LSP spec, the Marketplace | `docs/CLIENT_BEHAVIOUR.md`, which is the only place such a fact is stated; everything else points at it | `core/spec/meta/client_behaviour_spec.rb` (the greppable rows against `vscode/node_modules`, and that nothing else restates one) |
| **A claim about Ruby's own semantics** — anywhere a comment, spec or record says what the interpreter does | the pasted session that carries it, which `docs/CODE_DISCIPLINE.md` requires and which is now **re-run**, so editing the claim without re-running it fails; a session that raises is written with a `rescue` so its point is output rather than a stack trace | `core/spec/meta/interpreter_sessions_spec.rb` and `scripts/check_interpreter_sessions.rb` (every session in the tree, compared against what Ruby prints — `024.220`) |
...
| **The Core↔Agent protocol** — a request, a notification, a limit, or cancellation | `docs/design/docs/05-protocol.md`. **A section describing something unimplemented says so in the section**, rather than reading as a specification somebody may implement | `scripts/check_protocol_doc.rb` and `core/spec/meta/protocol_doc_spec.rb`, added in 0.3.1 — the version in the document against the Agent's `PROTOCOL_VERSION`, a section for every request the Agent dispatches, and, for every section naming something `core/lib` does not contain, the words saying so. All three were false when it was written: the document said version 1 against 2, omitted `agent/gemIndex`, `agent/status` and `agent/ancestors`, and specified seven methods that do not exist, six of them written as an ordinary specification |
| **A working agreement** (how this project is built, reviewed or released) | `AGENTS.md` — the one card, which `CLAUDE.md` imports — and the document behind the line: `docs/DEVELOPMENT.md`, `docs/CODE_DISCIPLINE.md`, `docs/MEASURING.md`, `docs/REVIEW_LOOP.md`. A rule's history goes to the register or the release record, never into the card (058) | `core/spec/meta/agents_card_spec.rb` (the card fits its budget, `CLAUDE.md` is exactly the import and its note, the card names no release or branch in any spelling this project has used and no task file beyond the two it reads by reference, and every document the card's index names declares a card section that exists — that it is the right section is not checked). The documents' prose is compared to nothing beyond that |
