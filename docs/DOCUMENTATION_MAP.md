...
| **A review round finding the same place the previous round did** | a mechanical countermeasure — a shared implementation, a rule moved to where the value is produced, a guard given the input it could not see — *not* a regression test for the one instance, and not a third hand fix | — for the judgement, which no scanner can make. The two halves that *are* mechanical are `scripts/review_round.rb`: `start <method>` refuses the previous round's method and refuses to open on a dirty tree, and `close` refuses when the index moved under the round. `core/spec/meta/review_round_spec.rb` |
| **A review round opened or closed** | the release's `docs/design/tasks/NNN-*.md`, through `ruby scripts/review_round.rb start <method>` — it writes the round's heading and an empty findings table there, and records the index the round reads in `core/tmp/` (untracked). `ruby scripts/review_round.rb close` refuses a round that ran while the index moved; findings that outlive the release go to the register, not into that table | `core/spec/meta/review_round_spec.rb`, and `core/spec/meta/task_findings_section_spec.rb` for the rule that a task document carrying findings says where findings actually go |
| **A deferred finding** (`024.*`) | its `yaml` metadata block in `docs/design/tasks/024-deferred-review-findings.md` (**the register is two files since 0.3.0's `024.R9`** — open entries there, resolved ones in `024-deferred-review-findings-resolved.md`, and `DeferredFindings.register` is the only reader that knows: anything reading the live file directly silently loses three-quarters of the entries. Resolving an entry means moving it across, and the generated index in the live file carries both) — `status`, and `released-in` once it is resolved; delete the entry once nothing in the tree still cites it by number — grep first, do not go by the calendar (see that file's own legend). Entries go in numeric order and the index at the head is generated: run `ruby scripts/reindex_findings.rb` rather than editing it. **Opening and closing an entry are commands, not hand edits**: `ruby scripts/issues.rb promote <n> …` writes one, `ruby scripts/issues.rb close 024.N --released-in V` resolves one — the second refuses while either language still publishes a paragraph for the finding, and `--drop-paragraphs` removes the whole `##` section rather than the marker | `core/spec/meta/deferred_findings_spec.rb` (status/kind/note/both-language anchors, **and that the order and index are current**) and `core/spec/meta/issues_tool_spec.rb` (the write primitive, and both refusals of each command) |
| **An issue nobody has triaged yet** (found in a review, a workflow, or by driving something) | the **Intake** section of [`docs/ISSUES.md`](ISSUES.md), which is where an issue starts. It moves into the register once it has been *driven* and its kind, target and user-visibility are known — `docs/ISSUES.md`'s own "The rule" says in what order. Do not open a register entry for something unverified: every field there is enforced, so an entry added early is an entry corrected later. `ruby scripts/issues.rb intake add` puts one here and `ruby scripts/issues.rb promote <n>` takes it out, by its position in the list `ruby scripts/issues.rb intake` prints | `core/spec/meta/issue_index_spec.rb` (the generated half is current with the register, and regenerating twice changes nothing) and `core/spec/meta/issues_tool_spec.rb` (promoting takes the item out of intake, and a refused promote spends no number) |
| **Install steps, prerequisites, or the extension id** | `README.md` + `.ja.md`, `docs/PUBLISHING.md` + `.ja.md`, `site/getting-started.html` + `site/ja/getting-started.html` | `core/spec/meta/extension_identity_spec.rb` (the id every install command and `itemName` link prints, against `vscode/package.json`, which is the only place it is true). The prose around it is not compared; the id is |
...
| **Where a release's work lives** (a branch created, renamed, renumbered, or work moved between branches) | the release's `NNN-*.md` on `main` names the branch; `AGENTS.md`'s current-loop pointer; `CONTRIBUTING.md` + `.ja.md`'s "Branches and pull requests" — 0.2.3 was prepared twice in parallel because this row did not exist (CLAUDE.md "Where a release's work lives", 028). **The convention is one branch per version merged by pull request** (`release/<version>`), set 2026-09-01; a change to it lands in all four | `scripts/check_release_pointers.rb` and `core/spec/meta/release_pointers_spec.rb`, added in 0.3.1: a `release/x.y.z` branch git can see that no task document names is 028's shape, and `release/0.3.1` was exactly that when the check was written. It reports rather than asserts when it can see no branches, because a tarball legitimately has none |
...
| **A working agreement** (how this project is built, reviewed or released) | `CLAUDE.md`, `AGENTS.md`, `CONTRIBUTING.md` + `.ja.md` | partly: `core/spec/meta/agents_restates_spec.rb` (every `AGENTS.md` bullet that restates a `CLAUDE.md` section names one that exists) and `agents_pointer_spec.rb` (the goal paragraph does not reacquire a release-and-branch claim). Neither reaches `CONTRIBUTING.md`, and the pointer guard itself went stale when the branch convention changed under it — it hunted `feat/` for two releases after that spelling was retired, fixed in 0.3.1 |
