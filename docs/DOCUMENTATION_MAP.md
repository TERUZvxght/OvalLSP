...
| **An issue nobody has triaged yet** (found in a review, a workflow, or by driving something) | the **Intake** section of [`docs/ISSUES.md`](ISSUES.md), which is where an issue starts. It moves into the register once it has been *driven* and its kind, target and user-visibility are known — `docs/ISSUES.md`'s own "The rule" says in what order. Do not open a register entry for something unverified: every field there is enforced, so an entry added early is an entry corrected later | `core/spec/meta/issue_index_spec.rb` (the generated half is current with the register, and regenerating twice changes nothing) |
