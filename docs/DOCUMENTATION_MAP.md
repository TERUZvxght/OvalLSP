...
| **A command id, setting, activation event, status-bar string, or workspace-trust declaration** | `vscode/package.json` is the source of truth; `docs/design/docs/07-vscode-extension.md`'s §3, §5, §6, §7 and §8 restate it and must be re-derived from it, not edited beside it | `core/spec/meta/design_doc_drift_spec.rb` (§3, §5, §6, §7 against `package.json` and `clientPresentation.ts` — set equality both ways) and `vscode/src/test/unit/workspaceTrust.test.ts` (§8's trust manifest, fail-closed). All five sections the row names are machine-checked |
...
| **The plugin API** — a registration method, the manifest, loading, or failure isolation | `docs/design/plugin-sdk.md` is the source of truth (public SDK, written with the implementation); `docs/design/schemas/plugin-manifest.schema.json`; `docs/design/docs/06-plugin-system.md` points at both rather than restating them | `core/spec/ovallsp/server_plugins_spec.rb` |
