...
| **A command id, setting, activation event, status-bar string, or workspace-trust declaration** | `vscode/package.json` is the source of truth; `docs/design/docs/07-vscode-extension.md`'s §3, §5, §6, §7 and §8 restate it and must be re-derived from it, not edited beside it | `vscode/src/test/unit/workspaceTrust.test.ts` (the trust manifest, fail-closed) — the rest is unchecked; see `046`'s C5 |
