...
- **`issues.rb promote` takes a list position that every promote shifts**
  - found by: scripts/issues.rb
  - The argument is an index into the intake listing, and promoting any item renumbers every item after it. Reading the list, choosing a number, then promoting is two commands with a shifting meaning between them -- done twice in one session, promoting R04 and then R06 while intending R03 and R13, each caught only by the register's own Area line looking wrong. A stable handle (the title, or a slug) would remove the class.
  - unverified: not yet driven against the tree
- **A constant written in `class << self` is indexed under the class, so an instance-side read of it is silenced**
  - found by: core/lib/ovallsp/parser_service.rb, core/lib/ovallsp/diagnostics/engine.rb
  - ruby -e 'class K; class << self; LIMIT = 1; end; def go = LIMIT; end; K.new.go' raises NameError: uninitialized constant K::LIMIT. #visit_constant_write_node records the constant with owner = current_owner, which inside a singleton body is still the class, so 024.330's reach counts it and the read is not reported. Before 024.330 it was reported, by accident -- no constant was resolvable at all. Found by cold review of bb1c450. Rare shape; the direction is silence, which is the safe one, and the fix is in how the index records a singleton-body constant rather than in the reach.
  - unverified: not yet driven against the tree
- **`WorkspaceIndex` never records a qualified top-level constant write under its namespace**
...
**50 open**, 292 resolved. 37 of the open ones are user-visible, and 38 are published in `KNOWN_LIMITATIONS` (38 in Japanese).
