...
- **`scripts/check_release_pointers.rb` takes any mention of a branch's
  name as naming it.** Seen in 057: a task document's record of the
...
- **`scripts/check_doc_links.rb`'s deletion marker is not restricted to
  records.** Seen in 057's attack round: on a live document's line the
...
- **A citation of a document's section is checked by nothing.** Seen in
  057: a changelog pointer named a `docs/CODE_DISCIPLINE.md` section that
...
**46 open**, 271 resolved. 34 of the open ones are user-visible, and 35 are published in `KNOWN_LIMITATIONS` (35 in Japanese).
