...
- **Four readers of the ancestor chain each write the same three decisions out by hand**
  ...
  - unverified: not yet driven against the tree
- **Six of DOCUMENTATION_MAP's twenty-one trigger rows still have nothing enforcing them**
  - found by: docs/DOCUMENTATION_MAP.md, the rows whose Checked by column reads as nothing
  - 0.3.1 mechanised two of the eight unenforced rows (the Core-Agent protocol, and where a release's work lives) and partly a third. Six remain, and they are not equal. Two are not mechanisable as stated -- a revert leaving documentation behind, and a review round finding the same place -- because both need a judgement about what counts as the same thing. Two more are cheap and have not drifted: the extension id agrees with package.json everywhere it appears, and SECURITY.md could get the parity check privacy_parity_spec already gives PRIVACY.md. One is cheap and HAS drifted: the Ruby patch versions named as tested were three in SUPPORT_MATRIX and two in both READMEs, corrected by hand in 0.3.1 with nothing to stop it happening again -- that row lists thirteen places. The last, which lock guards what, would take the shape rescue_verdicts.yml already proves: enumerate every Mutex in core/lib and require each to be accounted for in the architecture document. Nothing here is driven beyond the census; what is measured is which rows are empty and which of them currently disagree.
  - unverified: not yet driven against the tree
<!-- intake: none -->
...
**49 open**, 252 resolved. 37 of the open ones are user-visible, and 38 are published in `KNOWN_LIMITATIONS` (38 in Japanese).
...
| [`024.305`](design/tasks/024-deferred-review-findings.md#024305-one-name-six-modules-and-the-index-keeps-the-empty-one) | defect | no | — | One name, six modules, and the index keeps the empty one |
