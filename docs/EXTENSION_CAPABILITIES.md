...
| W2 | Rename one a macro declared (`attr_accessor`, `delegate`, …) | it is refused with a reason, not half-applied — there is no identifier token to rewrite, and editing only the call sites would leave the declaration behind (024.28) | PASS |
