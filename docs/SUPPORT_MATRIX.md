...
| 4.0 (run under 4.0.6) | best effort | `core/`'s suite is green under 4.0.6 — 1,875 examples, 0 failures, with the 76 real-Rails integration examples pending because Rails and sqlite3 are not installed for that Ruby on this machine. **Best effort means the library, not the artifact**: the VSIX bundles native extensions built for one Ruby version and `platformCompatibility.ts` requires an exact match, so a user whose Ruby is 4.0.x gets the same refusal a user on 3.3 gets. Nothing continuously verifies it either — `.github/workflows/ci.yml` runs `["3.3", "3.4"]` |
