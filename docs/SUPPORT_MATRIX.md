...
| Version | Tier | Basis |
|---|---|---|
| 3.4 (run under 3.4.5 and 3.4.7) | supported | `core/`'s test suite (1,953 examples — `core/spec/meta/documented_counts_spec.rb` compares this number against the running suite, because the figure was 895 for six releases, then 1,776 taken mid-branch, then 1,833 with two commits still to come) actually run and green on this machine. The interpreter is whichever ran the suite -- Homebrew's `ruby@3.4`, 3.4.10 at the time of writing. Only the example count is machine-checked; a specific patch version named beside it is not verified unless it has been re-measured |
