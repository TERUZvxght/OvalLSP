...
| 位置 | 変わるとき | 例 |
|---|---|---|
| patch (`0.1.**7**`) | 新しく告知するものが無い。**過去のいずれかのリリース**がすでに主張していたことを、実際にできるようにするリリース | 不具合修正、性能改善、リファクタリング、文書修正。**ケイパビリティ行の追加や ✅ 化はあり得る** — 前のリリースが当然やっていると理解されていて実際にはやっていなかったことを指す行であれば。0.2.1 は `G17` をこの形で追加した(0.2.0 が行を付けずに出した機能のため)。やってはいけないのは、誰にも約束していないケイパビリティを告知すること — 0.2.1 はレビュー中にそういう行を3つ足し、出荷前に3つともロードマップへ戻した |
...
**「前のリリース」ではなく「過去のいずれかのリリース」。** パッチは、
製品がこれまでに行ったどの主張との差でも埋めてよい。0.2.14 まではここが
「前のリリース」と書かれており、その読みではパッチがほぼ成立しません:
`0.N.0` の欠陥を見つける唯一の経路は `0.N+1` の作業中であり、その時点で
欠陥は2つ前のリリースのものになるからです。不具合修正の行き先が無くなり
ます。バージョン位置が答えるのは**この変更が何であるか**であって、
**どのリリースが原因か**ではありません。

### マイナーが未完成のまま出してよい範囲

**マイナーリリースは、`target:` を持たない未解決の user-visible 欠陥を
ゼロにして出荷する。** 直さないものは、直すリリースを名指すか、
`NOT YET` 行にする — 後者は「拡張はこれをやると主張しない」と明言する
ことです。

これが上のパッチ定義を成立させる条件です。あの定義は「過去のリリースが
実際より多く主張していた」ことを前提にしており、その差の**大きさ**を
縛るものが無かった。だからパッチ列は偶発ではなく構造的に保証され、
各 `0.N.0` は既知の壊れを抱えて出荷してよいことに暗黙になっていました。

規則が無かった 0.2.14 時点の実測: **target を全く持たない未解決の
user-visible 欠陥が18件**(`024.153`)。「直す時期を誰も決めていない制限」
として公開されていました。この条件はその状態を到達不能にします。

マイナーが無欠陥になるという意味ではありません。出荷**後**に見つかる
欠陥は避けようがなく、section 0 は「精度を追って1.0.0が遠のく方が、
追っている欠陥より悪い」と明言しています。禁じるのは、user-visible な
穴を**承知のうえで未割り当てのまま**機能を出荷することです —
「誰も見つけていなかった欠陥」と「誰も割り当てていない欠陥」の違いです。
...
できません。これが守っているのは、**プロジェクトオーナーが「このリリース
を出す」と判断していない限りpublishが起きないこと**です — 初回に限らず、
publishのたびに。無人で走るscriptに承認を焼き込んでおくこと、それがこの
プロンプトの防いでいる対象です。

一方で、**オーナー自身の指が打鍵することまでは要求していません**。0.2.3
はオーナーの明示的な指示のもとエージェントがこのscriptを実行して公開され
ましたが、これはオーナー自身が述べ直したとおり規則の範囲内です — 人間が
goサインを出し、publishが確実かつ安全に遂行される。その途中にエージェント
が介在することは問題なく、ビルドやsmokeの出力から異常を拾うという点では
人がスクロールして読むより適しています。範囲外なのは、判断を伴わずにこの
プロンプトへ到達すること、あるいは到達して自分で判断してしまうことです。

したがって義務は消えるのではなく移ります。オーナーに代わってプロンプトに
答える者は、その手前でscriptが出力したもの — vendoring検査、payload hash、
`vsce ls --tree`、semantic smoke、SHA-256 — を読んだ上で、何を見たかを報告
しなければなりません。0.2.3の実行記録は
`docs/design/tasks/028-0.2.3-review-loop.md`にあり、報告に値した唯一の事項
(`EBADENGINE`警告9件。すべて`@vscode/vsce`の依存とテストハーネス1件で、
出荷物には1つも含まれない)も併記しています。

このscript経由であれ、`vsce publish`を直接実行するのであれ、**以下の全てが
真になるまで実行してはいけません:**
